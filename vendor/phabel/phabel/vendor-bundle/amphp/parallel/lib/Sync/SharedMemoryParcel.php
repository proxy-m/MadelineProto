...
use PhabelVendor\Amp\Sync\PosixSemaphore;
use PhabelVendor\Amp\Sync\SyncException;
...
final class SharedMemoryParcel implements Parcel
{
    ...
    */
    public static function create(string $id, $value, int $size = 8192, int $permissions = 384, ?Serializer $serializer = NULL) : self
    {
     ...
     * @throws \Error If the size or permissions are invalid.
     */
    private function init($value, int $size = 8192, int $permissions = 384) : void
    {
        if ($size <= 0) {
    ...
    */
    private function wrap($value) : void
    {
        ...
        if (\shmop_size($this->handle) < $size + self::MEM_DATA_OFFSET) {
            $this->key = $this->key < 4294967295.0 ? $this->key + 1 : \random_int(0x10, 0.0);
