...
use PhabelVendor\PhpParser\Node\Scalar\Encapsed;
...
use PhabelVendor\PhpParser\Node\Stmt\Enum_;
use PhabelVendor\PhpParser\Node\Stmt\Interface_;
use PhabelVendor\PhpParser\Node\Stmt\Namespace_;
use PhabelVendor\PhpParser\Node\Stmt\Property;
use PhabelVendor\PhpParser\Node\Stmt\TryCatch;
    ...
    */
    ...
    protected function stripIndentation(string $string, int $indentLen, string $indentChar, bool $newlineAtStart, bool $newlineAtEnd, array $attributes)
    {
        ...
        return \preg_replace_callback($regex, function ($matches) use($indentLen, $indentChar, $attributes) {
            $prefix = \Phabel\Target\Php80\Polyfill::substr($matches[1], 0, $indentLen);
            ...
            return \Phabel\Target\Php80\Polyfill::substr($matches[0], \strlen($prefix));
        }, $string);
    }
    /**
     *
