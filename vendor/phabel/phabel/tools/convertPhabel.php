...
foreach ($target === 'all' ? Php::VERSIONS : [$target] as $realTarget) {
    ...
    $packages = (new Traverser(EventHandler::create()))->setInput('../phabelConvertedOutput')->setOutput('../phabelConvertedOutput')->setPlugins([Php::class => ['target' => $realTarget]])->setCoverage($coverage)->run((int) (\getenv('PHABEL_PARALLEL') ?: 1));
    \chdir($home);
    ...
    foreach ([Php::VERSIONS[\count(Php::VERSIONS) - 1], $realTarget] as $k => $target) {
        ...
        (new Traverser(EventHandler::create()))->setInput($input)->setOutput($output)->setPlugins([Php::class => ['target' => $target]])->setCoverage($coverage)->run((int) (\getenv('PHABEL_PARALLEL') ?: 1));
        \chdir($output);
        if (!$dry) {
    ...
    \file_put_contents('vendor/symfony/string/AbstractString.php', \str_replace('\\PREG_UNMATCHED_AS_NULL', "0", \file_get_contents('vendor/symfony/string/AbstractString.php')));
    $replace = 'array_walk_recursive($match, function (&$v){return $v === "" ? null : $v;})';
    \file_put_contents('vendor/symfony/string/AbstractUnicodeString.php', \str_replace(['return $matches;', '\\PREG_UNMATCHED_AS_NULL'], ["{$replace}; return \$matches;", "0"], \file_get_contents('vendor/symfony/string/AbstractUnicodeString.php')));
    \file_put_contents('vendor/symfony/string/ByteString.php', \str_replace(['return $matches;', '\\PREG_UNMATCHED_AS_NULL'], ["{$replace}; return \$matches;", "0"], \file_get_contents('vendor/symfony/string/ByteString.php')));
    \file_put_contents('src/Composer/Plugin.php', \str_replace('PhabelVendor\\Symfony', 'Symfony', \file_get_contents('src/Composer/Plugin.php')));
    \file_put_contents('src/Composer/Transformer.php', \str_replace('PhabelVendor\\Symfony', 'Symfony', \file_get_contents('src/Composer/Transformer.php')));
