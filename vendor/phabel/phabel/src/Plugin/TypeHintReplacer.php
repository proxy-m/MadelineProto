...
class TypeHintReplacer extends Plugin
{
    ...
    private const PRECEDENCE = array('string' => 3, 'float' => 2, 'int' => 1, 'bool' => 0);
    /**
     * Stack.
     *
     * @var SplStack
    ...
    private function generateConditions(Context $ctx, PhpDocNode $phpdoc, $param, array $types, ?Expr $className, bool $fromNullable = false) : array
    {
        ...
        foreach ($conditions as $condition) {
            if (\is_array($condition)) {
                ...
                $splitConditions[] = fn(Node ...$stmts): If_ => (new If_($conditionsLoose, ['stmts' => $stmts, 'else' => new Else_([new Expression(new Assign($var, new $castLoose($var)))])]));
            } else {
