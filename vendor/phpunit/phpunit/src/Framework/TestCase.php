...
abstract class TestCase extends Assert implements Reorderable, SelfDescribing, Test
{
    ...
    public function run(TestResult $result = NULL) : TestResult
    {
        ...
        if (!$this instanceof ErrorTestCase && !$this instanceof WarningTestCase && !$this instanceof SkippedTestCase && !$this->handleDependencies()) {
            return $result;
        }
    ...
    {
        if (trim($this->name) === '') {
            throw new Exception('PHPUnit\\Framework\\TestCase::$name must be a non-blank string.');
        }
     ...
     */
    protected function getMockClass(string $originalClassName, $methods = array(), array $arguments = array(), string $mockClassName = '', bool $callOriginalConstructor = false, bool $callOriginalClone = true, bool $callAutoload = true, bool $cloneArguments = false) : string
    {
    ...
    private function markSkippedForNotSpecifyingDependency() : void
    {
        ...
        $this->result->addError($this, new SkippedTestError('This method has an invalid @depends annotation.'), 0);
    ...
    private function markSkippedForMissingDependency(ExecutionOrderDependency $dependency) : void
    {
        $this->status = BaseTestRunner::STATUS_SKIPPED;
        $this->result->startTest($this);
        $this->result->addError($this, new SkippedTestError(sprintf('This test depends on "%s" to pass.', $dependency->getTarget())), 0);
        $this->result->endTest($this, 0);
    }
    private function markWarningForUncallableDependency(ExecutionOrderDependency $dependency) : void
    {
        $this->status = BaseTestRunner::STATUS_WARNING;
        $this->result->startTest($this);
        $this->result->addWarning($this, new Warning(sprintf('This test depends on "%s" which does not exist.', $dependency->getTarget())), 0);
        $this->result->endTest($this, 0);
    }
    /**
     ...
     * @throws \SebastianBergmann\RecursionContext\InvalidArgumentException
     */
    private function registerMockObjectsFromTestArguments(array $testArguments, array &$visited = array()) : void
    {
