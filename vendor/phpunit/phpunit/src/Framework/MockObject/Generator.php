...
{
    private const MOCKED_CLONE_METHOD_WITH_VOID_RETURN_TYPE_TRAIT = 'namespace PHPUnit\\Framework\\MockObject;
...
trait MockedCloneMethodWithVoidReturnType
{
    ...
    private const MOCKED_CLONE_METHOD_WITHOUT_RETURN_TYPE_TRAIT = 'namespace PHPUnit\\Framework\\MockObject;
...
trait MockedCloneMethodWithoutReturnType
{
    ...
    private const UNMOCKED_CLONE_METHOD_WITH_VOID_RETURN_TYPE_TRAIT = 'namespace PHPUnit\\Framework\\MockObject;
...
trait UnmockedCloneMethodWithVoidReturnType
{
    ...
    private const UNMOCKED_CLONE_METHOD_WITHOUT_RETURN_TYPE_TRAIT = 'namespace PHPUnit\\Framework\\MockObject;
...
trait UnmockedCloneMethodWithoutReturnType
{
     ...
     * @var array
     */
    private const EXCLUDED_METHOD_NAMES = array('__CLASS__' => true, '__DIR__' => true, '__FILE__' => true, '__FUNCTION__' => true, '__LINE__' => true, '__METHOD__' => true, '__NAMESPACE__' => true, '__TRAIT__' => true, '__clone' => true, '__halt_compiler' => true);
    /**
     * @var array
     */
     ...
     * @throws UnknownTypeException
     */
    public function getMockForAbstractClass(string $originalClassName, array $arguments = array(), string $mockClassName = '', bool $callOriginalConstructor = true, bool $callOriginalClone = true, bool $callAutoload = true, array $mockedMethods = NULL, bool $cloneArguments = true) : MockObject
    {
     ...
     * @throws UnknownTypeException
     */
    public function getMockForTrait(string $traitName, array $arguments = array(), string $mockClassName = '', bool $callOriginalConstructor = true, bool $callOriginalClone = true, bool $callAutoload = true, array $mockedMethods = NULL, bool $cloneArguments = true) : MockObject
    {
     ...
     * @throws UnknownTraitException
     */
    public function getObjectForTrait(string $traitName, string $traitClassName = '', bool $callAutoload = true, bool $callOriginalConstructor = false, array $arguments = array()) : object
    {
        if (!trait_exists($traitName, $callAutoload)) {
            throw new UnknownTraitException($traitName);
        }
        ...
        return $this->getObject(new MockTrait($classTemplate->render(), $className['className']), '', $callOriginalConstructor, $callAutoload, $arguments);
    }
    /**
     * @throws ClassIsFinalException
     * @throws ReflectionException
     ...
     * @throws RuntimeException
     */
    private function getObject(MockType $mockClass, $type = '', bool $callOriginalConstructor = false, bool $callAutoload = false, array $arguments = array(), bool $callOriginalMethods = false, object $proxyTarget = NULL, bool $returnValueGeneration = true)
    {
    ...
    private function generateMock(string $type, ?array $explicitMethods, string $mockClassName, bool $callOriginalClone, bool $callAutoload, bool $cloneArguments, bool $callOriginalMethods) : MockClass
    {
        ...
        if (!$isClass && !$isInterface) {
            ...
            if ($isInterface && $class->implementsInterface(Throwable::class)) {
                ...
                foreach ($this->userDefinedInterfaceMethods($_mockClassName['fullClassName']) as $method) {
                    ...
                    $mockMethods->addMethods(MockMethod::fromReflection($method, $callOriginalMethods, $cloneArguments));
                }
        ...
        if (is_array($explicitMethods)) {
            foreach ($explicitMethods as $methodName) {
                if ($class !== null && $class->hasMethod($methodName)) {
                    ...
                    $mockMethods->addMethods(MockMethod::fromName($_mockClassName['fullClassName'], $methodName, $cloneArguments));
                }
            }
        }
        $mockedMethods = '';
        ...
        return new MockClass($classTemplate->render(), $_mockClassName['className'], $configurable);
    }
    ...
    private function generateMockClassDeclaration(array $mockClassName, bool $isInterface, array $additionalInterfaces = array()) : string
    {
        $buffer = 'class ';
