...
class TestSuite implements IteratorAggregate, Reorderable, SelfDescribing, Test
{
    ...
    public function __construct($theClass = '', string $name = '')
    {
        if (!is_string($theClass) && !$theClass instanceof ReflectionClass) {
            throw InvalidArgumentException::create(1, 'ReflectionClass object or string');
        }
        ...
        if ($constructor !== null && !$constructor->isPublic()) {
            $this->addTest(new WarningTestCase(sprintf('Class "%s" has no public constructor.', $theClass->getName())));
            return;
        }
        ...
        if (empty($this->tests)) {
            $this->addTest(new WarningTestCase(sprintf('No tests found in class "%s".', $theClass->getName())));
        }
        $this->testCase = true;
    }
    /**
     ...
     * @param array $groups
     */
    public function addTest(Test $test, $groups = array()) : void
    {
        try {
    ...
    public function addTestSuite($testClass) : void
    {
        if (!(is_object($testClass) || is_string($testClass) && class_exists($testClass))) {
            throw InvalidArgumentException::create(1, 'class name or object');
        }
            ...
            try {
                ...
                foreach ($this->tests() as $test) {
                    ...
                    if (!$errorAdded) {
                        ...
                        $result->addFailure($test, new SkippedTestError('Test skipped because of an error in hook method'), 0);
                    }
                    $result->endTest($test, 0);
                }
                $result->endTestSuite($this);
