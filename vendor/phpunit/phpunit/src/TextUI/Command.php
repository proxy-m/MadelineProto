...
class Command
{
    ...
    protected function handleArguments(array $argv) : void
    {
        ...
        if (isset($this->arguments['configuration'])) {
            ...
            if (!isset($this->arguments['test'])) {
                try {
                    $this->arguments['test'] = (new TestSuiteMapper())->map($this->arguments['configurationObject']->testSuite(), $this->arguments['testsuite'] ?? '');
                } catch (Exception $e) {
                    $this->printVersionString();
                    print $e->getMessage() . PHP_EOL;
                    exit(TestRunner::EXCEPTION_EXIT);
    ...
    protected function handlePrinter(string $printerClass, string $printerFile = '')
    {
        ...
        if (!$class->implementsInterface(ResultPrinter::class)) {
            $this->exitWithErrorMessage(sprintf('Could not use "%s" as printer: class does not implement %s', $printerClass, ResultPrinter::class));
        }
