...
final class Loader
{
    ...
    public function logging(string $filename, DOMXPath $xpath) : Logging
    {
        ...
        if ($element) {
            $text = new Text(new File($this->toAbsolutePath($filename, (string) $this->getStringAttribute($element, 'outputFile'))));
        }
        $teamCity = null;
        ...
        if ($element) {
            $testDoxXml = new TestDoxXml(new File($this->toAbsolutePath($filename, (string) $this->getStringAttribute($element, 'outputFile'))));
        }
    ...
    public function legacyLogging(string $filename, DOMXPath $xpath) : Logging
    {
        ...
        foreach ($xpath->query('logging/log') as $log) {
            ...
            switch ($type) {
                case 'plain':
                    $text = new Text(new File($target));
                    break;
                case 'junit':
                    $junit = new Junit(new File($target));
                    break;
                case 'teamcity':
                    $teamCity = new TeamCity(new File($target));
                    break;
                case 'testdox-html':
                    $testDoxHtml = new TestDoxHtml(new File($target));
                    break;
                case 'testdox-text':
                    $testDoxText = new TestDoxText(new File($target));
                    break;
                case 'testdox-xml':
                    $testDoxXml = new TestDoxXml(new File($target));
                    break;
            }
        }
        return new Logging($junit, $text, $teamCity, $testDoxHtml, $testDoxText, $testDoxXml);
    ...
    private function getElementConfigurationParameters(string $filename, DOMElement $element) : Extension
    {
        ...
        if ($element->getAttribute('file')) {
            $file = $this->toAbsolutePath($filename, (string) $element->getAttribute('file'), true);
        }
    ...
    private function codeCoverage(string $filename, DOMXPath $xpath, DOMDocument $document) : CodeCoverage
    {
        ...
        if ($element) {
            $html = new CodeCoverageHtml(new Directory($this->toAbsolutePath($filename, (string) $this->getStringAttribute($element, 'outputDirectory'))), $this->getIntegerAttribute($element, 'lowUpperBound', 50), $this->getIntegerAttribute($element, 'highLowerBound', 90));
        }
        $php = null;
    ...
    private function legacyCodeCoverage(string $filename, DOMXPath $xpath, DOMDocument $document) : CodeCoverage
    {
        ...
        if ($element) {
            if ($element->hasAttribute('addUncoveredFilesFromWhitelist')) {
                $includeUncoveredFiles = (bool) $this->getBoolean((string) $element->getAttribute('addUncoveredFilesFromWhitelist'), true);
            }
        ...
        foreach ($xpath->query('logging/log') as $log) {
            ...
            switch ($type) {
                case 'coverage-clover':
                    $clover = new Clover(new File($target));
                    break;
                case 'coverage-cobertura':
                    $cobertura = new Cobertura(new File($target));
                    break;
                case 'coverage-crap4j':
                    $crap4j = new Crap4j(new File($target), $this->getIntegerAttribute($log, 'threshold', 30));
                    break;
                case 'coverage-html':
                    $html = new CodeCoverageHtml(new Directory($target), $this->getIntegerAttribute($log, 'lowUpperBound', 50), $this->getIntegerAttribute($log, 'highLowerBound', 90));
                    break;
                case 'coverage-php':
                    $php = new CodeCoveragePhp(new File($target));
                    break;
                case 'coverage-text':
                    $text = new CodeCoverageText(new File($target), $this->getBooleanAttribute($log, 'showUncoveredFiles', false), $this->getBooleanAttribute($log, 'showOnlySummary', false));
                    break;
                case 'coverage-xml':
                    $xml = new CodeCoverageXml(new Directory($target));
                    break;
            }
        }
    ...
    private function php(string $filename, DOMXPath $xpath) : Php
    {
        ...
        foreach ($xpath->query('php/ini') as $ini) {
            ...
            $iniSettings[] = new IniSetting((string) $ini->getAttribute('name'), (string) $ini->getAttribute('value'));
        }
        $constants = [];
        foreach ($xpath->query('php/const') as $const) {
            assert($const instanceof DOMElement);
            $value = (string) $const->getAttribute('value');
            $constants[] = new Constant((string) $const->getAttribute('name'), $this->getBoolean($value, $value));
        }
    ...
    private function testSuite(string $filename, DOMXPath $xpath) : TestSuiteCollection
    {
        ...
        foreach ($this->getTestSuiteElements($xpath) as $element) {
            ...
            $testSuites[] = new TestSuiteConfiguration((string) $element->getAttribute('name'), TestDirectoryCollection::fromArray($directories), TestFileCollection::fromArray($files), FileCollection::fromArray($exclude));
        }
