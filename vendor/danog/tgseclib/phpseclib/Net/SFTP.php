...
class SFTP extends SSH2
{
    ...
    public function __construct($host, $port = 22, $timeout = 10)
    {
        ...
        $this->attributes = [
            ...
            -1 << 31 & 4294967295.0 => 'NET_SFTP_ATTR_EXTENDED',
        ];
    ...
    public function login($username, ...$args)
    {
        ...
        if (!$this->send_sftp_packet(NET_SFTP_INIT, "\x00\x00\x00\x03")) {
            return false;
        }
    ...
    {
        $attr = pack('N3', NET_SFTP_ATTR_SIZE, $new_size / 4294967296.0, $new_size);
    ...
    public function put($remote_file, $data, $mode = 2, $start = -1, $local_start = -1, $progressCallback = NULL)
    {
        ...
        $sent = 0;
        $size = $size < 0 ? ($size & 0x7fffffff) + 2147483648.0 : $size;
        ...
        while ($dataCallback || ($size === 0 || $sent < $size)) {
            ...
            $subtemp = $offset + $sent;
            $packet = pack('Na*N3a*', strlen($handle), $handle, $subtemp / 4294967296.0, $subtemp, strlen($temp), $temp);
    ...
    public function get($remote_file, $local_file = false, $offset = 0, $length = -1, $progressCallback = NULL)
    {
        ...
        while (true) {
            ...
            while ($i < NET_SFTP_QUEUE_SIZE && ($length < 0 || $read < $length)) {
                ...
                $packet = Strings::packSSH2('sN3', $handle, $tempoffset / 4294967296.0, $tempoffset, $packet_size);
        ...
        foreach ($this->attributes as $key => $value) {
            switch ($flags & $key) {
                case NET_SFTP_ATTR_SIZE:
                    ...
                    $attr['size'] = $upper ? 4294967296.0 * $upper : 0;
                    $attr['size'] += $size < 0 ? ($size & 0x7fffffff) + 2147483648.0 : $size;
                    break;
