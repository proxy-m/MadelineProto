...
class FilesEntry
{
    ...
    private function getClassesInFile($options)
    {
        ...
        list($initialInterfaces, $initialClasses, $initialTraits, $initialFuncs) = $this->execReturn("var_export(array(get_declared_interfaces(), get_declared_classes(), get_declared_traits(), get_defined_functions()['user']));");
        // This brings in the new classes. so $classes here will include the PHP defaults and the newly defined classes
        list($allInterfaces, $allClasses, $allTraits, $allFuncs) = $this->execReturn("require_once '{$this->file}'; var_export(array(get_declared_interfaces(), get_declared_classes(), get_declared_traits(), get_defined_functions()['user']));");
