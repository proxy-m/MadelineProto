...
final class PgSqlHandle implements Handle
{
    ...
    public function statementExecute(string $name, array $params) : Promise
    {
        return call(function () use($name, $params) {
            ...
            return $this->createResult(yield from $this->send("pg_send_execute", $name, $params), $this->statements[$name]->sql, (yield $this->types));
        });
    }
    /**
     * @param string $name
    ...
    public function statementDeallocate(string $name) : Promise
    {
        if (!$this->isAlive()) {
            return new Success();
            // Connection closed, no need to deallocate.
        }
    ...
    public function execute(string $sql, array $params = array()) : Promise
    {
        ...
        return call(function () use($sql, $params) {
            return $this->createResult(yield from $this->send("pg_send_query_params", $sql, $params), $sql, (yield $this->types));
        });
    }
    /**
     * {@inheritdoc}
