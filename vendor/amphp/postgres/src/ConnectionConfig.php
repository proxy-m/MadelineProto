...
final class ConnectionConfig extends SqlConnectionConfig
{
    ...
    public const SSL_MODES = array(0 => 'disable', 1 => 'allow', 2 => 'prefer', 3 => 'require', 4 => 'verify-ca', 5 => 'verify-full');
    /** @var string|null */
    ...
    public static function fromString(string $connectionString) : self
    {
        ...
        $config = new self($parts["host"], (int) ($parts["port"] ?? self::DEFAULT_PORT), $parts["user"] ?? null, $parts["password"] ?? null, $parts["db"] ?? null);
        if (isset($parts["sslmode"])) {
            $config = $config->withSslMode($parts["sslmode"]);
        }
        return $config;
    }
    public function __construct(string $host, int $port = 5432, string $user = NULL, string $password = NULL, string $database = NULL)
    {
    ...
    public function getConnectionString() : string
    {
        ...
        $chunks = ["host=" . $this->getHost(), "port=" . $this->getPort()];
        $user = $this->getUser();
        if ($user !== null) {
            $chunks[] = "user=" . $user;
        }
