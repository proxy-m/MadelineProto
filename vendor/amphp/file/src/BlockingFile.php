...
final class BlockingFile implements File
{
    ...
    public function read(int $length = self::DEFAULT_READ_LENGTH) : Promise
    {
        ...
        return new Failure(new StreamException("Failed writing to file handle: " . ($error['message'] ?? 'Unknown error')));
    }
    /**
     * {@inheritdoc}
     */
    ...
    public function truncate(int $size) : Promise
    {
        ...
        if (!@\ftruncate($this->fh, $size)) {
            $error = \error_get_last();
            return new Failure(new StreamException("Could not truncate file: " . ($error['message'] ?? 'Unknown error')));
        }
        return new Success();
    }
    /**
        ...
        switch ($whence) {
            ...
            case \SEEK_END:
                if (@\fseek($this->fh, $position, $whence) === -1) {
                    $error = \error_get_last();
                    return new Failure(new StreamException("Could not seek in file: " . ($error['message'] ?? 'Unknown error')));
                }
