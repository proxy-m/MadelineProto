...
namespace PhabelVendor\Amp\Parser;
        ...
        try {
            while ($this->buffer !== "") {
                if (\is_int($this->delimiter)) {
                    ...
                    $send = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $this->delimiter);
                    $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $this->delimiter);
                } elseif (\is_string($this->delimiter)) {
                    if (($position = \strpos($this->buffer, $this->delimiter)) === \false) {
                        break;
                    }
                    $send = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $position);
                    $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $position + \strlen($this->delimiter));
                } else {
                    $send = $this->buffer;
                    $this->buffer = "";
                }
