    ...
    private function tokenize(string $input) : array
    {
        ...
        while ($cursor < $length) {
            ...
            if (\preg_match('/\\s+/A', $input, $match, 0, $cursor)) {
                ...
                $token .= $match[1] . $match[2] . \stripcslashes(\str_replace(['"\'', '\'"', '\'\'', '""'], '', \Phabel\Target\Php80\Polyfill::substr($match[3], 1, -1)));
            } elseif (\preg_match('/' . self::REGEX_QUOTED_STRING . '/A', $input, $match, 0, $cursor)) {
                $token .= \stripcslashes(\Phabel\Target\Php80\Polyfill::substr($match[0], 1, -1));
