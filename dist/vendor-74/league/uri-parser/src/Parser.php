...
';
    ...
    const URI_COMPONENTS = array('scheme' => NULL, 'user' => NULL, 'pass' => NULL, 'host' => NULL, 'port' => NULL, 'path' => '', 'query' => NULL, 'fragment' => NULL);
    /** @deprecated 1.4.0 will be removed in the next major point release */
    ...
    {
        if ('[' !== ($host[0] ?? '') || ']' !== \Phabel\Target\Php80\Polyfill::substr($host, -1)) {
            return false;
        }
        $ip = \Phabel\Target\Php80\Polyfill::substr($host, 1, -1);
        ...
        static $gen_delims = '/[:\\/?#\\[\\]@ ]/';
        // Also includes space.
        if (1 === preg_match($gen_delims, rawurldecode(\Phabel\Target\Php80\Polyfill::substr($ip, $pos)))) {
            return false;
        }
        $ip = \Phabel\Target\Php80\Polyfill::substr($ip, 0, $pos);
        if (!filter_var($ip, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6)) {
            return false;
        }
    ...
    */
    public function parse(string $uri) : array
    {
        ...
        if ('#' === $first_char) {
            ...
            $components['fragment'] = (string) \Phabel\Target\Php80\Polyfill::substr($uri, 1);
            return $components;
        }
        //The URI is made of the query and fragment
        if ('?' === $first_char) {
            $components = self::URI_COMPONENTS;
            list($components['query'], $components['fragment']) = explode('#', \Phabel\Target\Php80\Polyfill::substr($uri, 1), 2) + [1 => null];
            return $components;
        }
    ...
    */
    protected function parseSchemeSpecificPart(string $uri) : array
    {
        ...
        $remaining_uri = (string) \Phabel\Target\Php80\Polyfill::substr($uri, 2);
        $components = self::URI_COMPONENTS;
    ...
    protected function parseHostname(string $hostname) : array
    {
        ...
        return [$this->filterHost(\Phabel\Target\Php80\Polyfill::substr($hostname, 0, $delimiter_offset)), $this->filterPort(\Phabel\Target\Php80\Polyfill::substr($hostname, ++$delimiter_offset))];
    }
    /**
    ...
    */
    protected function parsePathQueryAndFragment(string $uri) : array
    {
        ...
        if (false !== ($pos = strpos($uri, ':')) && false === strpos(\Phabel\Target\Php80\Polyfill::substr($uri, 0, $pos), '/')) {
    ...
    */
    ...
    {
        trigger_error(self::class . '::' . __METHOD__ . ' is deprecated and will be removed in the next major point release', E_USER_DEPRECATED);
        return '' != $label && 63 >= strlen($label) && strlen($label) == \Phabel\Target\Php80\Polyfill::strspn($label, self::LABEL_VALID_STARTING_CHARS . '-_~' . self::SUB_DELIMITERS);
    }
    /**
    ...
    {
        trigger_error(self::class . '::' . __METHOD__ . ' is deprecated and will be removed in the next major point release', E_USER_DEPRECATED);
        if ('[' !== ($ipv6[0] ?? '') || ']' !== \Phabel\Target\Php80\Polyfill::substr($ipv6, -1)) {
            return false;
        }
        $ipv6 = \Phabel\Target\Php80\Polyfill::substr($ipv6, 1, -1);
        if (false === ($pos = strpos($ipv6, '%'))) {
            return (bool) filter_var($ipv6, FILTER_VALIDATE_IP, FILTER_FLAG_IPV6);
        }
        $scope = rawurldecode(\Phabel\Target\Php80\Polyfill::substr($ipv6, $pos));
        if (strlen($scope) !== \Phabel\Target\Php80\Polyfill::strcspn($scope, '?#@[]')) {
            return false;
        }
        $ipv6 = \Phabel\Target\Php80\Polyfill::substr($ipv6, 0, $pos);
