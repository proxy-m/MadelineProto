...
final class Rfc6455Client implements Client
{
    ...
    private function read() : \Generator
    {
        ...
        try {
            while (($chunk = (yield $this->socket->read())) !== null) {
                ...
                if ((self::$framesReadInLastSecond[$this->metadata->id] ?? 0) >= $maxFramesPerSecond || (self::$bytesReadInLastSecond[$this->metadata->id] ?? 0) >= $maxBytesPerSecond) {
                    /** @psalm-suppress PossiblyNullArgument */
        ...
        switch ($opcode) {
            case Opcode::CLOSE:
                ...
                if ($length === 0) {
                    ...
                    $code = \current(\unpack('n', \Phabel\Target\Php80\Polyfill::substr($data, 0, 2)));
                    $reason = \Phabel\Target\Php80\Polyfill::substr($data, 2);
        ...
        try {
            if (\strlen($data) > $this->options->getFrameSplitThreshold()) {
                ...
                for ($i = 0; $i < $slices - 1; ++$i) {
                    $chunk = \Phabel\Target\Php80\Polyfill::substr($data, $length * $i, $length);
                    if ($compress) {
                ...
                $data = \Phabel\Target\Php80\Polyfill::substr($data, $length * $i, $length);
            }
            if ($compress) {
                /** @psalm-suppress PossiblyNullReference */
                ...
                if ($this->nextMessageDeferred) {
                    ...
                    switch ($code) {
                        ...
                        case Code::NONE:
                            ...
                            break;
                        default:
                            $deferred->fail(new ClosedException('Connection closed abnormally while awaiting message', $code, $reason));
                            break;
                    }
                }
                (yield $promise);
                // Wait for writing close frame to complete.
                if ($this->closeDeferred !== null) {
    ...
    private function parser() : \Generator
    {
        ...
        while (true) {
            ...
            while ($bufferSize < 2) {
                $buffer = \Phabel\Target\Php80\Polyfill::substr($buffer, $offset);
                $offset = 0;
                $buffer .= yield;
                $bufferSize = \strlen($buffer);
            }
            ...
            if ($frameLength === 0x7e) {
                ...
                $lengthLong32Pair = \unpack('N2', \Phabel\Target\Php80\Polyfill::substr($buffer, $offset, 8));
                $offset += 8;
                $bufferSize -= 8;
            ...
            if ($isMasked) {
                ...
                $maskingKey = \Phabel\Target\Php80\Polyfill::substr($buffer, $offset, 4);
                $offset += 4;
                $bufferSize -= 4;
            }
            ...
            $payload = \Phabel\Target\Php80\Polyfill::substr($buffer, $offset, $frameLength);
            $buffer = \Phabel\Target\Php80\Polyfill::substr($buffer, $offset + $frameLength);
            $offset = 0;
            $bufferSize = \strlen($buffer);
            if ($isMasked) {
            ...
            if ($doUtf8Validation) {
                if ($final) {
                    ...
                    for ($i = 0; !($valid = \preg_match('//u', $payload)); $i++) {
                        $savedBuffer = \Phabel\Target\Php80\Polyfill::substr($payload, -1) . $savedBuffer;
                        $payload = \Phabel\Target\Php80\Polyfill::substr($payload, 0, -1);
                        if ($i === 3) {
