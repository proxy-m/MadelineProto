...
';
    private const DEFAULT_MAX_FRAME_SIZE = 16384;
    private const DEFAULT_WINDOW_SIZE = 65535;
    private const MINIMUM_WINDOW = 524288;
    private const WINDOW_INCREMENT = 1048576;
    ...
    */
    ...
    {
        $message = \sprintf("Received GOAWAY frame on '%s' from '%s' with error code %d", (string) $this->socket->getLocalAddress(), (string) $this->socket->getRemoteAddress(), $error);
        /**
    ...
    public function handleHeaders(int $streamId, array $pseudo, array $headers, bool $streamEnded) : void
    {
        ...
        $cancellationToken->subscribe(function (CancelledException $exception) use($streamId) : void {
            ...
            if (!$this->streams[$streamId]->originalCancellation->isRequested()) {
                ...
                $this->ping();
                // async ping, if other requests occur, they wait for it
    ...
    {
        if ($pushId % 2 === 1) {
            $this->handleConnectionException(new Http2ConnectionException("Invalid server initiated stream", Http2Parser::PROTOCOL_ERROR));
            return;
        }
        foreach ($pseudo as $name => $value) {
            if (!isset(Http2Parser::KNOWN_REQUEST_PSEUDO_HEADERS[$name])) {
                throw new Http2StreamException("Invalid pseudo header", $pushId, Http2Parser::PROTOCOL_ERROR);
            }
        }
        if (!isset($pseudo[":method"], $pseudo[":path"], $pseudo[":scheme"], $pseudo[":authority"]) || isset($headers["connection"]) || $pseudo[":path"] === '' || isset($headers["te"]) && \implode($headers["te"]) !== "trailers") {
            $this->handleStreamException(new Http2StreamException("Invalid header values", $pushId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        ...
        if ($method !== 'GET' && $method !== 'HEAD') {
            $this->handleStreamException(new Http2StreamException("Pushed request method must be a safe method", $pushId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        if (!\preg_match("#^([A-Z\\d.\\-]+|\\[[\\d:]+])(?::([1-9]\\d*))?\$#i", $host, $matches)) {
            $this->handleStreamException(new Http2StreamException("Invalid pushed authority (host) name", $pushId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        $host = $matches[1];
        $port = isset($matches[2]) ? (int) $matches[2] : $this->socket->getRemoteAddress()->getPort();
        if (!isset($this->streams[$streamId])) {
            $this->handleStreamException(new Http2StreamException("Parent stream {$streamId} is no longer open", $pushId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        ...
        if (\strcasecmp($host, $parentStream->request->getUri()->getHost()) !== 0) {
            $this->handleStreamException(new Http2StreamException("Authority does not match original request authority", $pushId, Http2Parser::PROTOCOL_ERROR));
            return;
        }
        if ($position = \strpos($target, "#")) {
            $target = \Phabel\Target\Php80\Polyfill::substr($target, 0, $position);
        }
        if ($position = \strpos($target, "?")) {
            $query = \Phabel\Target\Php80\Polyfill::substr($target, $position + 1);
            $target = \Phabel\Target\Php80\Polyfill::substr($target, 0, $position);
        }
        try {
        ...
        $stream = new Http2Stream($pushId, $request, HttpStream::fromStream($parentStream->stream, static function () {
            throw new \Error('Calling Stream::request() on a pushed request is forbidden');
        }, static function () {
            // nothing to do
        }), $parentStream->cancellationToken, $parentStream->originalCancellation, $this->createStreamInactivityWatcher($pushId, $request->getInactivityTimeout()), self::DEFAULT_WINDOW_SIZE, 0);
        ...
        if ($parentStream->request->getPushHandler() === null) {
            $this->handleStreamException(new Http2StreamException("Push promise refused", $pushId, Http2Parser::CANCEL));
            return;
        }
        asyncCall(function () use($pushId, $stream) : \Generator {
            ...
            $cancellationId = $cancellationToken->subscribe(function (CancelledException $exception) use($pushId) : void {
                if (!isset($this->streams[$pushId])) {
                    return;
                }
                $this->writeFrame(Http2Parser::RST_STREAM, Http2Parser::NO_FLAG, $pushId, \pack("N", Http2Parser::CANCEL));
    ...
    public function handleConnectionException(Http2ConnectionException $exception) : void
    {
         ...
         * @noinspection PhpDeprecationInspection
         */
        $this->shutdown(new ClientHttp2ConnectionException($exception->getMessage(), $exception->getCode(), $exception));
        $this->close();
    }
    /**
     *
    ...
    public function request(Request $request, CancellationToken $cancellationToken, Stream $stream) : Promise
    {
        return call(function () use($request, $cancellationToken, $stream) : \Generator {
            if ($this->shutdown !== null) {
                $exception = new UnprocessedRequestException(new SocketException(\sprintf("Connection from '%s' to '%s' has already been shut down", (string) $this->socket->getLocalAddress(), (string) $this->socket->getRemoteAddress())));
            ...
            if ($this->hasTimeout && !(yield $this->ping())) {
                $exception = new UnprocessedRequestException(new SocketException(\sprintf("Socket to '%s' missed responding to PINGs", (string) $this->socket->getRemoteAddress())));
                foreach ($request->getEventListeners() as $eventListener) {
                    (yield $eventListener->abort($request, $exception));
                }
                throw $exception;
            ...
            if ($this->socket->isClosed()) {
                $exception = new UnprocessedRequestException(new SocketException(\sprintf("Socket to '%s' closed before the request could be sent", (string) $this->socket->getRemoteAddress())));
                foreach ($request->getEventListeners() as $eventListener) {
                    (yield $eventListener->abort($request, $exception));
                }
                throw $exception;
            ...
            try {
                ...
                $this->streams[$streamId] = $http2stream = new Http2Stream($streamId, $request, $stream, $cancellationToken, $originalCancellation, $this->createStreamInactivityWatcher($streamId, $request->getInactivityTimeout()), self::DEFAULT_WINDOW_SIZE, $this->initialWindowSize);
                $this->socket->reference();
                $transferTimeout = $request->getTransferTimeout();
                $cancellationId = $cancellationToken->subscribe(function (CancelledException $exception) use($streamId, $originalCancellation, $transferTimeout) : void {
                    if (!isset($this->streams[$streamId])) {
                        return;
                    }
                    $this->writeFrame(Http2Parser::RST_STREAM, Http2Parser::NO_FLAG, $streamId, \pack("N", Http2Parser::CANCEL));
    ...
    private function run() : \Generator
    {
        try {
             ...
             * @noinspection PhpDeprecationInspection
             */
            $this->shutdown(new ClientHttp2ConnectionException('The HTTP/2 connection closed' . ($this->shutdown !== null ? ' unexpectedly' : ''), $this->shutdown ?? Http2Parser::INTERNAL_ERROR));
            $this->close();
            return;
        }
        try {
             ...
             * @noinspection PhpDeprecationInspection
             */
            $this->shutdown(new ClientHttp2ConnectionException("The HTTP/2 connection from '" . $this->socket->getLocalAddress() . "' to '" . $this->socket->getRemoteAddress() . "' closed" . ($this->shutdown === null ? ' unexpectedly' : ''), $this->shutdown ?? Http2Parser::INTERNAL_ERROR));
            $this->close();
        } catch (\Throwable $exception) {
            /**
             * @psalm-suppress DeprecatedClass
             * @noinspection PhpDeprecationInspection
             */
            $this->shutdown(new ClientHttp2ConnectionException("The HTTP/2 connection from '" . $this->socket->getLocalAddress() . "' to '" . $this->socket->getRemoteAddress() . "' closed unexpectedly: " . $exception->getMessage(), Http2Parser::INTERNAL_ERROR, $exception));
            $this->close();
        }
    }
    /**
     *
     */
    private function writeFrame(int $type, int $flags = 0, int $stream = 0, string $data = '') : Promise
    {
        \assert(Http2Parser::logDebugFrame('send', $type, $flags, $stream, \strlen($data)));
        return $this->frameQueueEmitter->emit(\Phabel\Target\Php80\Polyfill::substr(\pack("NccN", \strlen($data), $type, $flags, $stream), 1) . $data);
    }
    /**
     *
     */
    ...
    private function writeBufferedData(Http2Stream $stream) : Promise
    {
        ...
        if ($windowSize > 0) {
            ...
            for ($off = 0; $off < $end; $off += $this->frameSizeLimit) {
                $this->writeFrame(Http2Parser::DATA, Http2Parser::NO_FLAG, $stream->id, \Phabel\Target\Php80\Polyfill::substr($data, $off, $this->frameSizeLimit));
            }
            $promise = $this->writeFrame(Http2Parser::DATA, Http2Parser::NO_FLAG, $stream->id, \Phabel\Target\Php80\Polyfill::substr($data, $off, $windowSize - $off));
            $stream->requestBodyBuffer = \Phabel\Target\Php80\Polyfill::substr($data, $windowSize);
            $stream->enableInactivityWatcher();
            return $promise;
        }
            ...
            asyncCall(static function () use($request, $deferredAndEmitter, $exception) {
                try {
                    ...
                    foreach ($deferredAndEmitter as $deferredOrEmitter) {
                        \assert($deferredOrEmitter !== null);
                        // TODO Find out why psalm reports this as nullable
    ...
    */
    ...
    private function createStreamInactivityWatcher(int $streamId, int $timeout) : ?string
    {
        ...
        $watcher = Loop::delay($timeout, function () use($streamId, $timeout) : void {
            ...
            $this->writeFrame(Http2Parser::RST_STREAM, Http2Parser::NO_FLAG, $streamId, \pack("N", Http2Parser::CANCEL));
            $this->releaseStream($streamId, new TimeoutException("Inactivity timeout exceeded, more than {$timeout} ms elapsed from last data received"));
        });
    ...
    private function runWriteThread() : \Generator
    {
        try {
             ...
             * @noinspection PhpDeprecationInspection
             */
            $this->shutdown(new ClientHttp2ConnectionException("The HTTP/2 connection closed unexpectedly: " . $exception->getMessage(), Http2Parser::INTERNAL_ERROR, $exception), \max(0, $this->streamId));
        }
    }
}