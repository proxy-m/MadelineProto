...
';
    ...
    public const KNOWN_RESPONSE_PSEUDO_HEADERS = array(':status' => true);
    public const KNOWN_REQUEST_PSEUDO_HEADERS = array(':method' => true, ':authority' => true, ':path' => true, ':scheme' => true);
    ...
    public const END_STREAM = 1;
    public const END_HEADERS = 4;
    public const PADDED = 8;
    public const PRIORITY_FLAG = 32;
    ...
    public const PUSH_PROMISE = 5;
    public const PING = 6;
    public const GOAWAY = 7;
    public const WINDOW_UPDATE = 8;
    public const CONTINUATION = 9;
    // Settings
    public const HEADER_TABLE_SIZE = 1;
    // 1 << 12
    public const ENABLE_PUSH = 2;
    // 1
    public const MAX_CONCURRENT_STREAMS = 3;
    // INF
    public const INITIAL_WINDOW_SIZE = 4;
    // 1 << 16 - 1
    public const MAX_FRAME_SIZE = 5;
    // 1 << 14
    public const MAX_HEADER_LIST_SIZE = 6;
    // INF
    // Error codes
    public const GRACEFUL_SHUTDOWN = 0;
    public const PROTOCOL_ERROR = 1;
    public const INTERNAL_ERROR = 2;
    public const FLOW_CONTROL_ERROR = 3;
    public const SETTINGS_TIMEOUT = 4;
    public const STREAM_CLOSED = 5;
    public const FRAME_SIZE_ERROR = 6;
    public const REFUSED_STREAM = 7;
    public const CANCEL = 8;
    public const COMPRESSION_ERROR = 9;
    public const CONNECT_ERROR = 10;
    public const ENHANCE_YOUR_CALM = 11;
    public const INADEQUATE_SECURITY = 12;
    public const HTTP_1_1_REQUIRED = 13;
    /**
     *
     */
...
');
    ...
    private function consume(int $bytes) : \Generator
    {
        ...
        $consumed = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $this->bufferOffset, $bytes);
        if ($bufferEnd > 2048) {
            $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $bufferEnd);
            $this->bufferOffset = 0;
        } else {
    ...
    private function parseDataFrame(string $frameBuffer, int $frameLength, int $frameFlags, int $streamId) : void
    {
        ...
        $header = $headerLength === 0 ? '' : \Phabel\Target\Php80\Polyfill::substr($frameBuffer, 0, $headerLength);
        ...
        $data = \Phabel\Target\Php80\Polyfill::substr($frameBuffer, $headerLength, $frameLength - $headerLength - $padding);
    ...
    private function parsePushPromise(string $frameBuffer, int $frameLength, int $frameFlags, int $streamId) : void
    {
        ...
        $header = \Phabel\Target\Php80\Polyfill::substr($frameBuffer, 0, $headerLength);
        $padding = $isPadded ? \ord($header[0]) : 0;
        ...
        $this->pushHeaderBlockFragment($pushId, \Phabel\Target\Php80\Polyfill::substr($frameBuffer, $headerLength, $frameLength - $headerLength - $padding));
    ...
    private function parseHeaders(string $frameBuffer, int $frameLength, int $frameFlags, int $streamId) : void
    {
        ...
        $this->pushHeaderBlockFragment($streamId, \Phabel\Target\Php80\Polyfill::substr($frameBuffer, $headerLength, $frameLength - $headerLength - $padding));
    ...
    private function parseSettings(string $frameBuffer, int $frameLength, int $frameFlags, int $streamId) : void
    {
        ...
        while ($frameLength > 0) {
            ...
            $settings[$key] = $value;
            $frameBuffer = \Phabel\Target\Php80\Polyfill::substr($frameBuffer, 6);
            $frameLength -= 6;
        }
