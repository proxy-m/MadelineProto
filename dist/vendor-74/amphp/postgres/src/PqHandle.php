...
final class PqHandle implements Handle
{
    ...
    public function __construct(pq\Connection $handle)
    {
        ...
        $this->poll = Loop::onReadable($this->handle->socket, static function ($watcher) use(&$deferred, &$lastUsedAt, &$listeners, &$handle) : void {
            ...
            if ($deferred === null) {
                return;
                // No active query, only notification listeners.
            }
            if ($handle->busy) {
                return;
                // Not finished receiving data, poll again.
            }
        ...
        $this->await = Loop::onWritable($this->handle->socket, static function ($watcher) use(&$deferred, &$listeners, &$handle) : void {
            try {
                if (!$handle->flush()) {
                    return;
                    // Not finished sending data, continue polling for writability.
                }
        ...
        switch ($result->status) {
            ...
            case pq\Result::COMMAND_OK:
                if ($handle instanceof pq\Statement) {
                    return $handle;
                    // Will be wrapped into a PqStatement object.
                }
            ...
            case pq\Result::SINGLE_TUPLE:
                ...
                return new PqUnbufferedResultSet(coroutine(\Closure::fromCallable([$this, 'fetch'])), $result, \Closure::fromCallable([$this, 'release']));
    ...
    {
        \assert($this->busy instanceof Deferred && $this->busy !== $this->deferred, "Connection in invalid state when releasing");
        while ($this->handle->busy && $this->handle->getResult()) {
        }
        $deferred = $this->busy;
        $this->busy = null;
     ...
     * {@inheritdoc}
     */
    public function execute(string $sql, array $params = array()) : Promise
    {
        if (!$this->handle) {
            throw new \Error("The connection to the database has been closed");
        }
    ...
    public function prepare(string $sql) : Promise
    {
        ...
        return call(function () use($sql) {
            ...
            try {
                $storage->statement = (yield $storage->promise = new Coroutine($this->send($sql, [$this->handle, "prepareAsync"], $name, $modifiedSql)));
            } catch (\Throwable $exception) {
                unset($this->statements[$name]);
                throw $exception;
            } finally {
    ...
    public function listen(string $channel) : Promise
    {
        return call(function () use($channel) {
            ...
            try {
                yield from $this->send(null, [$this->handle, "listenAsync"], $channel, static function (string $channel, string $message, int $pid) use($emitter) {
