...
final class ArrayParser
{
    ...
    private function parser(string $data, callable $cast, string $delimiter = ',') : \Generator
    {
        ...
        $data = \ltrim(\Phabel\Target\Php80\Polyfill::substr($data, 1));
        do {
            if ($data === '') {
                throw new ParseException("Unexpected end of data");
            }
            if ($data[0] === '}') {
                // Empty array
                return \ltrim(\Phabel\Target\Php80\Polyfill::substr($data, 1));
            }
            if ($data[0] === '{') {
                // Array
                $parser = $this->parser($data, $cast, $delimiter);
            ...
            if ($data[0] === '"') {
                ...
                $yield = \stripslashes(\Phabel\Target\Php80\Polyfill::substr($data, 1, $position - 1));
                ...
                $yield = \trim(\Phabel\Target\Php80\Polyfill::substr($data, 0, $position));
                $end = $this->trim($data, $position, $delimiter);
                if (\strcasecmp($yield, "NULL") === 0) {
                    // Literal NULL is always unquoted.
                    (yield null);
                    continue;
                }
            }
    ...
    {
        $data = \ltrim(\Phabel\Target\Php80\Polyfill::substr($data, $position));
        if ($data === '') {
            throw new ParseException("Unexpected end of data");
        }
        $end = $data[0];
