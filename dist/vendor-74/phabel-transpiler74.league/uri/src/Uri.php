...
final class Uri implements UriInterface
{
     ...
     */
    private const SCHEME_DEFAULT_PORT = array('data' => NULL, 'file' => NULL, 'ftp' => 21, 'gopher' => 70, 'http' => 80, 'https' => 443, 'ws' => 80, 'wss' => 443);
    /**
     * URI validation methods per scheme.
     *
     * @var array<string>
     */
    private const SCHEME_VALIDATION_METHOD = array('data' => 'isUriWithSchemeAndPathOnly', 'file' => 'isUriWithSchemeHostAndPathOnly', 'ftp' => 'isNonEmptyHostUriWithoutFragmentAndQuery', 'gopher' => 'isNonEmptyHostUriWithoutFragmentAndQuery', 'http' => 'isNonEmptyHostUri', 'https' => 'isNonEmptyHostUri', 'ws' => 'isNonEmptyHostUriWithoutFragment', 'wss' => 'isNonEmptyHostUriWithoutFragment');
    /**
...
g|hv.fb,:=-q10C2*yx)(L9AS/P"EjMIk3>5T<D4}B{8FwR67UGN;JzV#HOW_&!K?XQ%Y\\	Z+~^$@`' . "\x00" . '';
    ...
    private function formatIp(string $host) : string
    {
        ...
        if (1 === preg_match(self::REGEXP_HOST_GEN_DELIMS, rawurldecode(\Phabel\Target\Php80\Polyfill::substr($ip, $pos)))) {
            throw new SyntaxError(sprintf('The host `%s` is invalid : the IP host is malformed', $host));
        }
    ...
    */
    public static function createFromComponents(array $components = array()) : self
    {
    ...
    public static function createFromWindowsPath(string $uri = '') : self
    {
        ...
        if (1 === preg_match(self::REGEXP_WINDOW_PATH, $uri, $matches)) {
            $root = \Phabel\Target\Php80\Polyfill::substr($matches['root'], 0, -1) . ':';
            $uri = \Phabel\Target\Php80\Polyfill::substr($uri, strlen($root));
        }
        $uri = str_replace('\\', '/', $uri);
        $uri = implode('/', array_map('rawurlencode', explode('/', $uri)));
        ...
        //UNC Windows Path
        if ('//' !== \Phabel\Target\Php80\Polyfill::substr($uri, 0, 2)) {
            return Uri::createFromComponents(['path' => $uri]);
        }
        $parts = explode('/', \Phabel\Target\Php80\Polyfill::substr($uri, 2), 2) + [1 => null];
    ...
    private static function fetchUserInfo(array $server) : array
    {
        ...
        if (0 === strpos(strtolower($server['HTTP_AUTHORIZATION']), 'basic')) {
            $userinfo = base64_decode(\Phabel\Target\Php80\Polyfill::substr($server['HTTP_AUTHORIZATION'], 6), true);
            if (false === $userinfo) {
    ...
    private function formatDataPath(string $path) : string
    {
        ...
        if (strlen($path) !== \Phabel\Target\Php80\Polyfill::strspn($path, self::ASCII) || false === strpos($path, ',')) {
    ...
    private function assertValidPath(string $mimetype, string $parameters, string $data) : void
    {
        ...
        if ($is_binary) {
            $parameters = \Phabel\Target\Php80\Polyfill::substr($parameters, 0, -strlen($matches[0]));
        }
    ...
    */
    ...
    private function assertValidState() : void
    {
        ...
        if (null === $this->authority && null === $this->scheme && false !== $pos && false === strpos(\Phabel\Target\Php80\Polyfill::substr($this->path, 0, $pos), '/')) {
