...
final class Rfc6455Connector implements Connector
{
     ...
     */
    public function __construct(HttpClient $client, ?ConnectionFactory $connectionFactory = NULL, ?CompressionContextFactory $compressionFactory = NULL)
    {
        $this->client = $client;
    ...
    public function connect(Handshake $handshake, ?CancellationToken $cancellationToken = NULL) : Promise
    {
        return call(function () use($handshake, $cancellationToken) {
            ...
            $request->setUpgradeHandler(static function (EncryptableSocket $socket, Request $request, Response $response) use($connectionFactory, $compressionFactory, $deferred, $key, $options) : void {
