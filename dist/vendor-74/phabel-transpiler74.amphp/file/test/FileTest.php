...
abstract class FileTest extends FilesystemTest
{
    ...
    public function testSequentialReads() : \Generator
    {
        ...
        $contents .= (yield $handle->read(10));
        $expected = \Phabel\Target\Php80\Polyfill::substr((yield $this->driver->read(__FILE__)), 0, 20);
    ...
    public function testReadingFromOffset() : \Generator
    {
        ...
        $expected = \Phabel\Target\Php80\Polyfill::substr((yield $this->driver->read(__FILE__)), 10, 90);
    ...
    public function testTruncateToLargerSize() : \Generator
    {
        ...
        $contents = (yield $handle->read());
        $this->assertTrue($handle->eof());
        $this->assertSame("foo\x00\x00\x00", $contents);
        $this->assertSame(3, (yield $handle->write("bar")));
        $this->assertSame(9, $handle->tell());
        (yield $handle->seek(0));
        $contents = (yield $handle->read());
        $this->assertSame("foo\x00\x00\x00bar", $contents);
        (yield $handle->close());
    }
    /**
     *
