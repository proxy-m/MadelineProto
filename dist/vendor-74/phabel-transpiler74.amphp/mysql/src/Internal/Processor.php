...
final class StatusFlags
{
    ...
    public const SERVER_MORE_RESULTS_EXISTS = 8;
    public const SERVER_STATUS_NO_GOOD_INDEX_USED = 16;
    public const SERVER_STATUS_NO_INDEX_USED = 32;
    public const SERVER_STATUS_CURSOR_EXISTS = 64;
    // Used by Binary Protocol Resultset to signal that COM_STMT_FETCH has to be used to fetch the row-data.
    public const SERVER_STATUS_LAST_ROW_SENT = 128;
    public const SERVER_STATUS_DB_DROPPED = 256;
    public const SERVER_STATUS_NO_BACKSLASH_ESCAPES = 512;
    public const SERVER_STATUS_METADATA_CHANGED = 1024;
    public const SERVER_QUERY_WAS_SLOW = 2048;
    public const SERVER_PS_OUT_PARAMS = 4096;
    public const SERVER_STATUS_IN_TRANS_READONLY = 8192;
    // in a read-only transaction
    public const SERVER_SESSION_STATE_CHANGED = 16384;
    // connection state information has changed
}
/** @see 13.1.3.1.1 Session State Information */
final class SessionStateTypes
{
    public const SESSION_TRACK_SYSTEM_VARIABLES = 0;
    public const SESSION_TRACK_SCHEMA = 1;
    public const SESSION_TRACK_STATE_CHANGE = 2;
    public const SESSION_TRACK_GTIDS = 3;
    public const SESSION_TRACK_TRANSACTION_CHARACTERISTICS = 4;
    public const SESSION_TRACK_TRANSACTION_STATE = 5;
}
/** @internal */
class Processor implements TransientResource
{
    const STATEMENT_PARAM_REGEX = '~(["\'`])(?:\\\\(?:\\\\|\\1)|(?!\\1).)*+\\1(*SKIP)(*FAIL)|(\\?)|:([a-zA-Z_][a-zA-Z0-9_]*)~ms';
    /** @var \Generator[] */
    private $processors = [];
    /** @var int */
    private $protocol;
    ...
    private const MAX_PACKET_SIZE = 16777215;
    private const MAX_UNCOMPRESSED_BUFLEN = 16777211;
    private const CLIENT_LONG_FLAG = 4;
    private const CLIENT_CONNECT_WITH_DB = 8;
    private const CLIENT_COMPRESS = 32;
    private const CLIENT_LOCAL_INFILE = 128;
    private const CLIENT_PROTOCOL_41 = 512;
    private const CLIENT_SSL = 2048;
    private const CLIENT_TRANSACTIONS = 8192;
    private const CLIENT_SECURE_CONNECTION = 32768;
    private const CLIENT_MULTI_STATEMENTS = 65536;
    private const CLIENT_MULTI_RESULTS = 131072;
    private const CLIENT_PS_MULTI_RESULTS = 262144;
    private const CLIENT_PLUGIN_AUTH = 524288;
    private const CLIENT_CONNECT_ATTRS = 1048576;
    private const CLIENT_SESSION_TRACK = 8388608;
    private const CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA = 2097152;
    private const CLIENT_DEPRECATE_EOF = 16777216;
    private const OK_PACKET = 0;
    private const EXTRA_AUTH_PACKET = 1;
    private const LOCAL_INFILE_REQUEST = 251;
    private const AUTH_SWITCH_PACKET = 254;
    private const EOF_PACKET = 254;
    private const ERR_PACKET = 255;
            ...
            while (($bytes = (yield $this->socket->read())) !== null) {
                ...
                \assert((function () use($bytes) {
                    if (\defined("MYSQL_DEBUG")) {
                        ...
                        \fwrite(STDERR, \str_replace($r, ".", \Phabel\Target\Php80\Polyfill::substr($bytes, 0, 200)) . "\n");
                    }
                    return true;
                })());
    ...
    public function setCharset(string $charset, string $collate = '') : Promise
    {
        return \Amp\call(function () use($charset, $collate) {
            if ($collate === "" && false !== ($off = \strpos($charset, "_"))) {
                $collate = $charset;
                $charset = \Phabel\Target\Php80\Polyfill::substr($collate, 0, $off);
            }
    ...
    public function execute(int $stmtId, string $query, array &$params, array $prebound, array $data = array()) : Promise
    {
        $deferred = new Deferred();
        $this->appendTask(function () use($stmtId, $query, &$params, $prebound, $data, $deferred) {
            ...
            if ($paramCount) {
                ...
                $payload .= \str_repeat("\x00", $paramCount + 7 >> 3);
    ...
    public function listFields(string $table, string $like = '%') : Promise
    {
        return $this->startCommand(static function () use($table, $like) {
            $this->sendPacket("\x04{$table}\x00{$like}");
    ...
    {
        // @TODO flags to use?
        $this->capabilities |= self::CLIENT_SESSION_TRACK | self::CLIENT_TRANSACTIONS | self::CLIENT_PROTOCOL_41 | self::CLIENT_SECURE_CONNECTION | self::CLIENT_MULTI_RESULTS | self::CLIENT_PS_MULTI_RESULTS | self::CLIENT_MULTI_STATEMENTS | self::CLIENT_PLUGIN_AUTH | self::CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA;
    ...
    {
        $off = 1;
        $this->connInfo->errorCode = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off, 2));
        $off += 2;
        if ($this->capabilities & self::CLIENT_PROTOCOL_41) {
            $this->connInfo->errorState = \Phabel\Target\Php80\Polyfill::substr($packet, $off, 6);
            $off += 6;
        }
        $this->connInfo->errorMsg = \Phabel\Target\Php80\Polyfill::substr($packet, $off);
    ...
    {
        $off = 1;
        $this->connInfo->affectedRows = DataTypes::decodeUnsigned(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $intlen);
        $off += $intlen;
        $this->connInfo->insertId = DataTypes::decodeUnsigned(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $intlen);
        $off += $intlen;
        if ($this->capabilities & (self::CLIENT_PROTOCOL_41 | self::CLIENT_TRANSACTIONS)) {
            $this->connInfo->statusFlags = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
            $off += 2;
            $this->connInfo->warnings = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
            $off += 2;
        }
        if ($this->capabilities & self::CLIENT_SESSION_TRACK) {
            ...
            if (\strlen($packet) > $off) {
                ...
                if ($this->connInfo->statusFlags & StatusFlags::SERVER_SESSION_STATE_CHANGED) {
                    $sessionState = DataTypes::decodeString(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $intlen, $sessionStateLen);
                    $len = 0;
                    while ($len < $sessionStateLen) {
                        $data = DataTypes::decodeString(\Phabel\Target\Php80\Polyfill::substr($sessionState, $len + 1), $datalen, $intlen);
                        switch ($type = DataTypes::decodeUnsigned8(\Phabel\Target\Php80\Polyfill::substr($sessionState, $len))) {
                            case SessionStateTypes::SESSION_TRACK_SYSTEM_VARIABLES:
                                $var = DataTypes::decodeString($data, $varintlen, $strlen);
                                $this->connInfo->sessionState[$type][$var] = DataTypes::decodeString(\Phabel\Target\Php80\Polyfill::substr($data, $varintlen + $strlen));
                                break;
            ...
            $this->connInfo->statusInfo = \Phabel\Target\Php80\Polyfill::substr($packet, $off);
        }
    }
    /**
     *
    ...
    {
        if ($this->capabilities & self::CLIENT_PROTOCOL_41) {
            $this->connInfo->warnings = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, 1));
            $this->connInfo->statusFlags = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, 3));
        }
    }
    /**
     *
    ...
    private function handleHandshake(string $packet) : void
    {
        ...
        $this->connInfo->serverVersion = DataTypes::decodeNullString(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $len);
        $off += $len + 1;
        $this->connectionId = DataTypes::decodeUnsigned32(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        $off += 4;
        $this->authPluginData = \Phabel\Target\Php80\Polyfill::substr($packet, $off, 8);
        $off += 8;
        $off += 1;
        // filler byte
        $this->serverCapabilities = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        $off += 2;
        if (\strlen($packet) > $off) {
            $this->connInfo->charset = \ord(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
            $off += 1;
            $this->connInfo->statusFlags = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
            $off += 2;
            $this->serverCapabilities += DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off)) << 16;
            $off += 2;
            $this->authPluginDataLen = $this->serverCapabilities & self::CLIENT_PLUGIN_AUTH ? \ord(\Phabel\Target\Php80\Polyfill::substr($packet, $off)) : 0;
            $off += 1;
            if ($this->serverCapabilities & self::CLIENT_SECURE_CONNECTION) {
                $off += 10;
                $strlen = \max(13, $this->authPluginDataLen - 8);
                $this->authPluginData .= \Phabel\Target\Php80\Polyfill::substr($packet, $off, $strlen);
                $off += $strlen;
                if ($this->serverCapabilities & self::CLIENT_PLUGIN_AUTH) {
                    $this->authPluginName = DataTypes::decodeNullString(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
                }
            }
        }
        $this->sendHandshake();
    ...
    private function handleAuthSwitch($packet)
    {
        ...
        $this->authPluginData = \Phabel\Target\Php80\Polyfill::substr($packet, $off);
    ...
    private function handleLocalInfileRequest(string $packet) : void
    {
        \Amp\asyncCall(function () use($packet) {
            try {
                $filePath = \Phabel\Target\Php80\Polyfill::substr($packet, 1);
    ...
    private function parseColumnDefinition(string $packet) : array
    {
        ...
        if ($this->capabilities & self::CLIENT_PROTOCOL_41) {
            ...
            $len = 0;
            $column["charset"] = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off + $len));
            $len += 2;
            $column["columnlen"] = DataTypes::decodeUnsigned32(\Phabel\Target\Php80\Polyfill::substr($packet, $off + $len));
            $len += 4;
            $column["type"] = \ord($packet[$off + $len]);
            $len += 1;
            $column["flags"] = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off + $len));
            $len += 2;
            ...
            $column["columnlen"] = DataTypes::decodeIntByLen(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $collen);
            $off += $collen;
            $typelen = DataTypes::decodeUnsignedOff($packet, $off);
            $column["type"] = DataTypes::decodeIntByLen(\Phabel\Target\Php80\Polyfill::substr($packet, $off), $typelen);
            $off += $typelen;
            $len = 1;
            $flaglen = $this->capabilities & self::CLIENT_LONG_FLAG ? DataTypes::decodeUnsigned(\Phabel\Target\Php80\Polyfill::substr($packet, $off, 9), $len) : \ord($packet[$off]);
            $off += $len;
            if ($flaglen > 2) {
                $len = 2;
                $column["flags"] = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off, 4));
            } else {
                $len = 1;
        ...
        if ($off < \strlen($packet)) {
            $column["defaults"] = DataTypes::decodeString(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        }
        return $column;
    }
    /**
    ...
    private function handleBinaryResultSetRow(string $packet) : void
    {
        ...
        for ($i = 0; $off < \strlen($packet); $i++) {
            ...
            $fields[$i] = DataTypes::decodeBinary($columns[$i]["type"], \Phabel\Target\Php80\Polyfill::substr($packet, $off), $len);
            $off += $len;
        }
        \ksort($fields);
        $this->result->rowFetched($fields);
    ...
    private function handlePrepare(string $packet) : void
    {
        ...
        $off = 1;
        $stmtId = DataTypes::decodeUnsigned32(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        $off += 4;
        $columns = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        $off += 2;
        $params = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
        $off += 2;
        $off += 1;
        // filler
        $this->connInfo->warnings = DataTypes::decodeUnsigned16(\Phabel\Target\Php80\Polyfill::substr($packet, $off));
    ...
    private function write(string $packet) : Promise
    {
        ...
        \assert((function () use($packet) {
            if (\defined("MYSQL_DEBUG")) {
                ...
                \fwrite(STDERR, \str_replace($r, ".", \Phabel\Target\Php80\Polyfill::substr($packet, 0, 200)) . "\n");
            }
            return true;
        })());
        // @codeCoverageIgnoreEnd
    ...
    private function compilePacket(string $pending) : string
    {
        ...
        do {
            $len = \strlen($pending);
            if ($len >= (1 << 24) - 1) {
                $out = \Phabel\Target\Php80\Polyfill::substr($pending, 0, (1 << 24) - 1);
                $pending = \Phabel\Target\Php80\Polyfill::substr($pending, (1 << 24) - 1);
                $len = (1 << 24) - 1;
            } else {
                $out = $pending;
                $pending = "";
            }
            $packet .= \Phabel\Target\Php80\Polyfill::substr_replace(\pack("V", $len), \chr(++$this->seqId), 3, 1) . $out;
            // expects $len < (1 << 24) - 1
        } while ($pending !== "");
        return $packet;
    }
    ...
    private function compressPacket(string $packet) : string
    {
        ...
        if ($len < \strlen($deflated)) {
            return \Phabel\Target\Php80\Polyfill::substr_replace(\pack("V", \strlen($packet)), \chr(++$this->compressionId), 3, 1) . "\x00\x00\x00" . $packet;
        }
        return \Phabel\Target\Php80\Polyfill::substr_replace(\pack("V", \strlen($deflated)), \chr(++$this->compressionId), 3, 1) . \Phabel\Target\Php80\Polyfill::substr(\pack("V", $len), 0, 3) . $deflated;
    }
    /**
    ...
    private function parseCompression() : \Generator
    {
        ...
        while (true) {
            ...
            $uncompressed = DataTypes::decodeUnsigned24(\Phabel\Target\Php80\Polyfill::substr($buf, 4, 3));
            $buf = \Phabel\Target\Php80\Polyfill::substr($buf, 7);
            if ($size > 0) {
                ...
                if ($uncompressed === 0) {
                    $inflated .= \Phabel\Target\Php80\Polyfill::substr($buf, 0, $size);
                } else {
                    $inflated .= \zlib_decode(\Phabel\Target\Php80\Polyfill::substr($buf, 0, $size), $uncompressed);
                }
                $buf = \Phabel\Target\Php80\Polyfill::substr($buf, $size);
            }
        }
    }
    /**
    ...
    private function parseMysql() : \Generator
    {
        ...
        while (true) {
            ...
            do {
                ...
                $buf = \Phabel\Target\Php80\Polyfill::substr($buf, 4);
                ...
                $packet .= \Phabel\Target\Php80\Polyfill::substr($buf, 0, $size);
                $buf = \Phabel\Target\Php80\Polyfill::substr($buf, $size);
            } while (!$lastIn);
            if (\strlen($packet) > 0) {
                $parsed[] = $packet;
                ...
                case self::EXTRA_AUTH_PACKET:
                    ...
                    switch ($this->authPluginName) {
                        case "sha256_password":
                            $key = \Phabel\Target\Php80\Polyfill::substr($packet, 1);
                            $this->config = $this->config->withKey($key);
                            $this->sendHandshake();
                            break;
                        case "caching_sha2_password":
                            switch (\ord(\Phabel\Target\Php80\Polyfill::substr($packet, 1, 1))) {
                                case 3:
                                    // success
                                    return;
                                // expecting OK afterwards
                                case 4:
                                    // fast auth failure
                                    if ($this->capabilities & self::CLIENT_SSL || $this->config->getHost()[0] === "/") {
                                        $this->write($this->config->getPassword() . "\x00");
                                    } else {
                                        $this->write("\x02");
                                    }
                                    break;
                                case 0x2d:
                                    // certificate
                                    $pubkey = \Phabel\Target\Php80\Polyfill::substr($packet, 1);
    ...
    {
        $hash = \sha1($pass, 1);
        return $hash ^ \sha1(\Phabel\Target\Php80\Polyfill::substr($scramble, 0, 20) . \sha1($hash, 1), 1);
    }
    /**
     *
     */
    ...
    {
        \openssl_public_encrypt("{$pass}\x00" ^ \str_repeat($scramble, \ceil(\strlen($pass) / \strlen($scramble))), $auth, $key, OPENSSL_PKCS1_OAEP_PADDING);
        return $auth;
    }
    /**
     *
    ...
    private function sha2Auth(string $pass, string $scramble) : string
    {
        ...
        return $hash ^ \hash("sha256", \Phabel\Target\Php80\Polyfill::substr($scramble, 0, 20) . \hash("sha256", $hash, true), true);
    }
    /**
     *
     */
    private function authSwitchRequest(string $packet) : void
    {
        ...
        switch (\ord($packet)) {
            case self::EOF_PACKET:
                ...
                $len = \strpos($packet, "\x00");
                $pluginName = \Phabel\Target\Php80\Polyfill::substr($packet, 0, $len);
                // @TODO mysql_native_pass only now...
                $authPluginData = \Phabel\Target\Php80\Polyfill::substr($packet, $len + 1);
    ...
    private function sendHandshake(bool $inSSL = false) : void
    {
        ...
        $payload .= $this->config->getUser() . "\x00";
        ...
        if ($this->capabilities & self::CLIENT_CONNECT_WITH_DB) {
            $payload .= "{$this->config->getDatabase()}\x00";
        }
        if ($this->capabilities & self::CLIENT_PLUGIN_AUTH) {
            $payload .= "{$this->authPluginName}\x00";
        }
