...
abstract class SymmetricKey
{
     ...
     */
    const MODE_MAP = array('ctr' => -1, 'ecb' => 1, 'cbc' => 2, 'cfb' => 3, 'cfb8' => 7, 'ofb' => 4, 'ofb8' => 8, 'gcm' => 5, 'stream' => 6);
    /**
     ...
     */
    const ENGINE_MAP = array(1 => 'PHP', 2 => 'Eval', 3 => 'mcrypt', 4 => 'OpenSSL', 5 => 'libsodium', 6 => 'OpenSSL (GCM)');
    /**
     * The Encryption Mode
     *
    ...
    */
                ...
                switch (true) {
                    case $method == 'pkcs12':
                        ...
                        */
                        ...
                        $s = \Phabel\Target\Php80\Polyfill::substr($s, 0, $blockLength);
                        $p = '';
                        if (strlen($password)) {
                        ...
                        $p = \Phabel\Target\Php80\Polyfill::substr($p, 0, $blockLength);
                        $i = $s . $p;
                    ...
                    case $method == 'pbkdf1':
                        ...
                        $key = \Phabel\Target\Php80\Polyfill::substr($t, 0, $dkLen);
                        $this->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $dkLen >> 1));
                        if ($this->usesIV()) {
                            $this->setIV(\Phabel\Target\Php80\Polyfill::substr($key, $dkLen >> 1));
                        }
                        return true;
                    case !in_array($hash, hash_algos()):
                        ...
                        $key = \Phabel\Target\Php80\Polyfill::substr($key, 0, $dkLen);
                        break;
                    default:
    ...
    private static function pkcs12helper($n, $hashObj, $i, $d, $count)
    {
        ...
        for ($j = 1; $j <= $c; $j++) {
            ...
            $b = \Phabel\Target\Php80\Polyfill::substr($b, 0, $blockLength);
            $b = new BigInteger($b, 256);
            $newi = '';
            for ($k = 0; $k < strlen($i); $k += $blockLength) {
                $temp = \Phabel\Target\Php80\Polyfill::substr($i, $k, $blockLength);
        ...
        return \Phabel\Target\Php80\Polyfill::substr($a, 0, $n);
    }
    /**
    * Encrypts a message.
    *
            ...
            switch ($this->mode) {
                ...
                case self::MODE_CBC:
                    ...
                    if ($this->continuousBuffer) {
                        $this->encryptIV = \Phabel\Target\Php80\Polyfill::substr($result, -$this->block_size);
                    }
                    return $result;
                case self::MODE_CTR:
                ...
                case self::MODE_CFB:
                    ...
                    if ($pos) {
                        ...
                        $ciphertext = \Phabel\Target\Php80\Polyfill::substr($iv, $orig_pos) ^ $plaintext;
                        $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, $ciphertext, $orig_pos, $i);
                        $plaintext = \Phabel\Target\Php80\Polyfill::substr($plaintext, $i);
                    }
                    $overflow = $len % $this->block_size;
                    if ($overflow) {
                        $ciphertext .= openssl_encrypt(\Phabel\Target\Php80\Polyfill::substr($plaintext, 0, -$overflow) . str_repeat("\x00", $this->block_size), $this->cipher_name_openssl, $this->key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
                        $iv = Strings::pop($ciphertext, $this->block_size);
                        $size = $len - $overflow;
                        $block = $iv ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, -$overflow);
                        $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, $block, 0, $overflow);
                        $ciphertext .= $block;
                        $pos = $overflow;
                    } elseif ($len) {
                        $ciphertext = openssl_encrypt($plaintext, $this->cipher_name_openssl, $this->key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
                        $iv = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$this->block_size);
                    }
                    return $ciphertext;
                case self::MODE_CFB8:
                    ...
                    if ($this->continuousBuffer) {
                        if (($len = strlen($ciphertext)) >= $this->block_size) {
                            $this->encryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$this->block_size);
                        } else {
                            $this->encryptIV = \Phabel\Target\Php80\Polyfill::substr($this->encryptIV, $len - $this->block_size) . \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len);
                        }
                    }
                    return $ciphertext;
                case self::MODE_OFB8:
                    ...
                    for ($i = 0; $i < $len; ++$i) {
                        ...
                        $iv = \Phabel\Target\Php80\Polyfill::substr($iv, 1) . $xor[0];
                    }
                    if ($this->continuousBuffer) {
                        $this->encryptIV = $iv;
                    }
            ...
            if ($this->mode == self::MODE_CFB && $this->continuousBuffer) {
                ...
                if ($len >= $block_size) {
                    if ($this->enbuffer['enmcrypt_init'] === false || $len > $this->cfb_init_len) {
                        ...
                        $ciphertext .= mcrypt_generic($this->enmcrypt, \Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $len - $len % $block_size));
                        $iv = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$block_size);
                        $len %= $block_size;
                    } else {
                        while ($len >= $block_size) {
                            $iv = mcrypt_generic($this->ecb, $iv) ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $block_size);
                            $ciphertext .= $iv;
                            $len -= $block_size;
                            $i += $block_size;
                        }
                ...
                if ($len) {
                    ...
                    $block = $iv ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, -$len);
                    $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, $block, 0, $len);
                    $ciphertext .= $block;
                    $pos = $len;
                }
        ...
        switch ($this->mode) {
            case self::MODE_ECB:
                for ($i = 0; $i < strlen($plaintext); $i += $block_size) {
                    $ciphertext .= $this->encryptBlock(\Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $block_size));
                }
                break;
            case self::MODE_CBC:
                $xor = $this->encryptIV;
                for ($i = 0; $i < strlen($plaintext); $i += $block_size) {
                    $block = \Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $block_size);
            ...
            case self::MODE_CTR:
                ...
                if ($this->continuousBuffer) {
                    ...
                    if ($start = strlen($plaintext) % $block_size) {
                        $buffer['ciphertext'] = \Phabel\Target\Php80\Polyfill::substr($key, $start) . $buffer['ciphertext'];
                    }
                }
                break;
            case self::MODE_CFB:
                ...
                while ($len >= $block_size) {
                    $iv = $this->encryptBlock($iv) ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $block_size);
                    $ciphertext .= $iv;
                    $len -= $block_size;
                    $i += $block_size;
                }
                if ($len) {
                    ...
                    $block = $iv ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, $i);
                    $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, $block, 0, $len);
                    $ciphertext .= $block;
                    $pos = $len;
                }
            ...
            case self::MODE_CFB8:
                ...
                for ($i = 0; $i < $len; ++$i) {
                    ...
                    $iv = \Phabel\Target\Php80\Polyfill::substr($iv, 1) . $c;
                }
                if ($this->continuousBuffer) {
                    if ($len >= $block_size) {
                        $this->encryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$block_size);
                    } else {
                        $this->encryptIV = \Phabel\Target\Php80\Polyfill::substr($this->encryptIV, $len - $block_size) . \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len);
                    }
                }
                break;
            case self::MODE_OFB8:
            ...
            case self::MODE_OFB:
                ...
                if (strlen($buffer['xor'])) {
                    ...
                    for ($i = 0; $i < strlen($plaintext); $i += $block_size) {
                        $xor = $this->encryptBlock($xor);
                        $ciphertext .= \Phabel\Target\Php80\Polyfill::substr($plaintext, $i, $block_size) ^ $xor;
                    }
                    $key = $xor;
                }
                if ($this->continuousBuffer) {
                    $this->encryptIV = $xor;
                    if ($start = strlen($plaintext) % $block_size) {
                        $buffer['xor'] = \Phabel\Target\Php80\Polyfill::substr($key, $start) . $buffer['xor'];
                    }
                }
                break;
            case self::MODE_STREAM:
    ...
    */
    public function decrypt($ciphertext)
    {
        ...
        if ($this->mode == self::MODE_GCM || isset($this->poly1305Key)) {
            ...
            if ($this->oldtag != \Phabel\Target\Php80\Polyfill::substr($newtag, 0, strlen($newtag))) {
                $cipher = clone $this;
                unset($cipher->poly1305Key);
                $this->usePoly1305 = false;
                $plaintext = $cipher->decrypt($ciphertext);
        ...
        if ($this->engine === self::ENGINE_OPENSSL) {
            switch ($this->mode) {
                ...
                case self::MODE_CBC:
                    ...
                    if ($this->continuousBuffer) {
                        $this->decryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$offset, $this->block_size);
                    }
                    break;
                case self::MODE_CTR:
                ...
                case self::MODE_CFB:
                    ...
                    if ($pos) {
                        ...
                        $plaintext = \Phabel\Target\Php80\Polyfill::substr($iv, $orig_pos) ^ $ciphertext;
                        $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, \Phabel\Target\Php80\Polyfill::substr($ciphertext, 0, $i), $orig_pos, $i);
                        $ciphertext = \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i);
                    }
                    $overflow = $len % $this->block_size;
                    if ($overflow) {
                        $plaintext .= openssl_decrypt(\Phabel\Target\Php80\Polyfill::substr($ciphertext, 0, -$overflow), $this->cipher_name_openssl, $this->key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $iv);
                        if ($len - $overflow) {
                            $iv = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$overflow - $this->block_size, -$overflow);
                        }
                        ...
                        $plaintext .= $iv ^ \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$overflow);
                        $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$overflow), 0, $overflow);
                        $pos = $overflow;
                    } elseif ($len) {
                ...
                case self::MODE_CFB8:
                    ...
                    if ($this->continuousBuffer) {
                        if (($len = strlen($ciphertext)) >= $this->block_size) {
                            $this->decryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$this->block_size);
                        } else {
                            $this->decryptIV = \Phabel\Target\Php80\Polyfill::substr($this->decryptIV, $len - $this->block_size) . \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len);
                        }
                    }
                    break;
                case self::MODE_OFB8:
        ...
        if ($this->engine === self::ENGINE_MCRYPT) {
            ...
            if ($this->mode == self::MODE_CFB && $this->continuousBuffer) {
                ...
                if ($len >= $block_size) {
                    $cb = \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i, $len - $len % $block_size);
                    $plaintext .= mcrypt_generic($this->ecb, $iv . $cb) ^ $cb;
                    $iv = \Phabel\Target\Php80\Polyfill::substr($cb, -$block_size);
                    $len %= $block_size;
                }
                if ($len) {
                    $iv = mcrypt_generic($this->ecb, $iv);
                    $plaintext .= $iv ^ \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len);
                    $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len), 0, $len);
                    $pos = $len;
                }
                restore_error_handler();
                return $plaintext;
        ...
        switch ($this->mode) {
            case self::MODE_ECB:
                for ($i = 0; $i < strlen($ciphertext); $i += $block_size) {
                    $plaintext .= $this->decryptBlock(\Phabel\Target\Php80\Polyfill::substr($ciphertext, $i, $block_size));
                }
                break;
            case self::MODE_CBC:
                $xor = $this->decryptIV;
                for ($i = 0; $i < strlen($ciphertext); $i += $block_size) {
                    $block = \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i, $block_size);
            ...
            case self::MODE_CFB:
                ...
                while ($len >= $block_size) {
                    $iv = $this->encryptBlock($iv);
                    $cb = \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i, $block_size);
                    $plaintext .= $iv ^ $cb;
                    $iv = $cb;
                    $len -= $block_size;
                    $i += $block_size;
                ...
                if ($len) {
                    $iv = $this->encryptBlock($iv);
                    $plaintext .= $iv ^ \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i);
                    $iv = \Phabel\Target\Php80\Polyfill::substr_replace($iv, \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i), 0, $len);
                    $pos = $len;
                }
                break;
            case self::MODE_CFB8:
                ...
                for ($i = 0; $i < $len; ++$i) {
                    ...
                    $iv = \Phabel\Target\Php80\Polyfill::substr($iv, 1) . $ciphertext[$i];
                }
                if ($this->continuousBuffer) {
                    if ($len >= $block_size) {
                        $this->decryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$block_size);
                    } else {
                        $this->decryptIV = \Phabel\Target\Php80\Polyfill::substr($this->decryptIV, $len - $block_size) . \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$len);
                    }
                }
                break;
            case self::MODE_OFB8:
            ...
            case self::MODE_OFB:
                ...
                if (strlen($buffer['xor'])) {
                    ...
                    for ($i = 0; $i < strlen($ciphertext); $i += $block_size) {
                        $xor = $this->encryptBlock($xor);
                        $plaintext .= \Phabel\Target\Php80\Polyfill::substr($ciphertext, $i, $block_size) ^ $xor;
                    }
                    $key = $xor;
                }
                if ($this->continuousBuffer) {
    ...
    public function getTag($length = 16)
    {
        ...
        return $length == 16 ? $this->newtag : \Phabel\Target\Php80\Polyfill::substr($this->newtag, 0, $length);
    }
    /**
    ...
    */
    private function openssl_ctr_process($plaintext, &$encryptIV, &$buffer)
    {
        ...
        if (strlen($buffer['ciphertext'])) {
            ...
            $plaintext = \Phabel\Target\Php80\Polyfill::substr($plaintext, strlen($ciphertext));
            if (!strlen($plaintext)) {
                return $ciphertext;
            }
        }
        ...
        if ($overflow) {
            ...
            if ($this->continuousBuffer) {
                $buffer['ciphertext'] = \Phabel\Target\Php80\Polyfill::substr($temp, $overflow);
                $encryptIV = $temp;
            }
    ...
    */
    private function openssl_ofb_process($plaintext, &$encryptIV, &$buffer)
    {
        if (strlen($buffer['xor'])) {
            ...
            $buffer['xor'] = \Phabel\Target\Php80\Polyfill::substr($buffer['xor'], strlen($ciphertext));
            $plaintext = \Phabel\Target\Php80\Polyfill::substr($plaintext, strlen($ciphertext));
        } else {
            $ciphertext = '';
        }
        ...
        if (strlen($plaintext)) {
            if ($overflow) {
                $ciphertext .= openssl_encrypt(\Phabel\Target\Php80\Polyfill::substr($plaintext, 0, -$overflow) . str_repeat("\x00", $block_size), $this->cipher_name_openssl, $key, OPENSSL_RAW_DATA | OPENSSL_ZERO_PADDING, $encryptIV);
                $xor = Strings::pop($ciphertext, $block_size);
                if ($this->continuousBuffer) {
                    $encryptIV = $xor;
                }
                $ciphertext .= Strings::shift($xor, $overflow) ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, -$overflow);
                if ($this->continuousBuffer) {
                    $buffer['xor'] = $xor;
                }
            } else {
                ...
                if ($this->continuousBuffer) {
                    $encryptIV = \Phabel\Target\Php80\Polyfill::substr($ciphertext, -$block_size) ^ \Phabel\Target\Php80\Polyfill::substr($plaintext, -$block_size);
                }
            }
        }
        return $ciphertext;
    ...
    */
    protected function unpad($text)
    {
        ...
        return \Phabel\Target\Php80\Polyfill::substr($text, 0, -$length);
    }
    /**
    ...
    */
    private function ghash($x)
    {
        ...
        foreach ($x as $xn) {
            ...
            $y[$n] = \Phabel\Target\Php80\Polyfill::substr($y[$n], 1);
        }
