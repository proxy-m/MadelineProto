...
class PublicKey extends RSA implements Common\PublicKey
{
    ...
    */
    private function rsassa_pkcs1_v1_5_relaxed_verify($m, $s)
    {
        ...
        $hash = \Phabel\Target\Php80\Polyfill::substr($hash, 0, 3) == 'id-' ? \Phabel\Target\Php80\Polyfill::substr($hash, 3) : $hash;
        $hash = new Hash($hash);
        $em = $hash->hash($m);
        $em2 = $decoded['digest'];
    ...
    private function emsa_pss_verify($m, $em, $emBits)
    {
        ...
        $maskedDB = \Phabel\Target\Php80\Polyfill::substr($em, 0, -$this->hLen - 1);
        $h = \Phabel\Target\Php80\Polyfill::substr($em, -$this->hLen - 1, $this->hLen);
        ...
        if (\Phabel\Target\Php80\Polyfill::substr($db, 0, $temp) != str_repeat(chr(0), $temp) || ord($db[$temp]) != 1) {
            return false;
        }
        $salt = \Phabel\Target\Php80\Polyfill::substr($db, $temp + 1);
        // should be $sLen long
