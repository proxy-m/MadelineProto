...
abstract class Barrett extends Base
{
    ...
    */
    protected static function reduce($n, $m)
    {
        ...
        $lsd = \Phabel\Target\Php80\Polyfill::substr($n, -$cutoff);
        $msd = \Phabel\Target\Php80\Polyfill::substr($n, 0, -$cutoff);
        $temp = bcmul($msd, $m1);
        // m.length + (m.length >> 1)
        $n = bcadd($lsd, $temp);
        ...
        $temp = \Phabel\Target\Php80\Polyfill::substr($n, 0, -$m_length + 1);
        // if even: ((m.length >> 1) + 2) + (m.length >> 1) == m.length + 2
        // if odd:  ((m.length >> 1) + 2) + (m.length >> 1) == (m.length - 1) + 2 == m.length + 1
        $temp = bcmul($temp, $u);
        // if even: (m.length + 2) - ((m.length >> 1) + 1) = m.length - (m.length >> 1) + 1
        // if odd:  (m.length + 1) - ((m.length >> 1) + 1) = m.length - (m.length >> 1)
        $temp = \Phabel\Target\Php80\Polyfill::substr($temp, 0, -($m_length >> 1) - 1);
    ...
    */
    private static function regularBarrett($x, $n)
    {
        ...
        $temp = \Phabel\Target\Php80\Polyfill::substr($x, 0, -$n_length + 1);
        $temp = bcmul($temp, $cache[self::DATA][$key]);
        $temp = \Phabel\Target\Php80\Polyfill::substr($temp, 0, -$n_length - 1);
        $r1 = \Phabel\Target\Php80\Polyfill::substr($x, -$n_length - 1);
        $r2 = \Phabel\Target\Php80\Polyfill::substr(bcmul($temp, $n), -$n_length - 1);
        $result = bcsub($r1, $r2);
        //if (bccomp($result, '0') < 0) {
        if ($result[0] == '-') {
