...
abstract class Strings
{
    ...
    {
        $substr = \Phabel\Target\Php80\Polyfill::substr($string, 0, $index);
        $string = \Phabel\Target\Php80\Polyfill::substr($string, $index);
        return $substr;
    }
    /**
     * String Pop
    ...
    {
        $substr = \Phabel\Target\Php80\Polyfill::substr($string, -$index);
        $string = \Phabel\Target\Php80\Polyfill::substr($string, 0, -$index);
        return $substr;
    }
    /**
    ...
    */
    ...
    private static function formatPack($format)
    {
        ...
        for ($i = 1; $i < count($parts); $i += 2) {
            $format .= \Phabel\Target\Php80\Polyfill::substr($parts[$i - 1], 0, -1) . str_repeat(\Phabel\Target\Php80\Polyfill::substr($parts[$i - 1], -1), $parts[$i]);
        }
        $format .= $parts[$i - 1];
        return $format;
    }
    ...
    */
    ...
    public static function increment_str(&$var)
    {
        ...
        for ($i = 4; $i <= strlen($var); $i += 4) {
            $temp = \Phabel\Target\Php80\Polyfill::substr($var, -$i, 4);
            switch ($temp) {
                case "\xff\xff\xff\xff":
                    $var = \Phabel\Target\Php80\Polyfill::substr_replace($var, "\x00\x00\x00\x00", -$i, 4);
                    break;
                case "\xff\xff\xff":
                    $var = \Phabel\Target\Php80\Polyfill::substr_replace($var, "\x80\x00\x00\x00", -$i, 4);
                    return $var;
                default:
                    $temp = unpack('Nnum', $temp);
                    $var = \Phabel\Target\Php80\Polyfill::substr_replace($var, pack('N', $temp['num'] + 1), -$i, 4);
                    return $var;
            }
        }
        ...
        $temp = unpack('Nnum', str_pad(\Phabel\Target\Php80\Polyfill::substr($var, 0, $remainder), 4, "\x00", STR_PAD_LEFT));
        $temp = \Phabel\Target\Php80\Polyfill::substr(pack('N', $temp['num'] + 1), -$remainder);
        $var = \Phabel\Target\Php80\Polyfill::substr_replace($var, $temp, 0, $remainder);
        return $var;
    }
    /**
