...
class PrivateKey extends RSA implements Common\PrivateKey
{
    ...
    */
    ...
    private function rsaes_pkcs1_v1_5_decrypt($c)
    {
        ...
        $ps = \Phabel\Target\Php80\Polyfill::substr($em, 2, strpos($em, chr(0), 2) - 2);
        $m = \Phabel\Target\Php80\Polyfill::substr($em, strlen($ps) + 3);
        if (strlen($ps) < 8) {
            throw new \RuntimeException('Decryption error');
        }
        // Output M
    ...
    */
    private function rsaes_oaep_decrypt($c)
    {
        ...
        $y = ord($em[0]);
        $maskedSeed = \Phabel\Target\Php80\Polyfill::substr($em, 1, $this->hLen);
        $maskedDB = \Phabel\Target\Php80\Polyfill::substr($em, $this->hLen + 1);
        $seedMask = $this->mgf1($maskedDB, $this->hLen);
        $seed = $maskedSeed ^ $seedMask;
        $dbMask = $this->mgf1($seed, $this->k - $this->hLen - 1);
        $db = $maskedDB ^ $dbMask;
        $lHash2 = \Phabel\Target\Php80\Polyfill::substr($db, 0, $this->hLen);
        $m = \Phabel\Target\Php80\Polyfill::substr($db, $this->hLen);
        ...
        return \Phabel\Target\Php80\Polyfill::substr($m, $offset + 1);
    }
    /**
