    ...
    */
    public static function string($length)
    {
        ...
        if ($crypto === false) {
            ...
            $old_session_id = \Phabel\Target\Php80\Polyfill::session_id();
            $old_use_cookies = ini_get('session.use_cookies');
            $old_session_cache_limiter = \Phabel\Target\Php80\Polyfill::session_cache_limiter();
            $_OLD_SESSION = isset($_SESSION) ? $_SESSION : false;
            if ($old_session_id != '') {
                session_write_close();
            }
            \Phabel\Target\Php80\Polyfill::session_id(1);
            ini_set('session.use_cookies', 0);
            \Phabel\Target\Php80\Polyfill::session_cache_limiter('');
            session_start();
            ...
            if ($old_session_id != '') {
                \Phabel\Target\Php80\Polyfill::session_id($old_session_id);
                session_start();
                ini_set('session.use_cookies', $old_use_cookies);
                \Phabel\Target\Php80\Polyfill::session_cache_limiter($old_session_cache_limiter);
            } else {
            ...
            $crypto->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $crypto->getKeyLength() >> 3));
            $crypto->setIV(\Phabel\Target\Php80\Polyfill::substr($iv, 0, $crypto->getBlockLength() >> 3));
        ...
        return \Phabel\Target\Php80\Polyfill::substr($result, 0, $length);
    }
    /**
