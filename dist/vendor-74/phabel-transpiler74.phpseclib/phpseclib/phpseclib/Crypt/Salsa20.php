...
class Salsa20 extends StreamCipher
{
    ...
    protected function createPoly1305Key()
    {
        ...
        $this->setPoly1305Key(\Phabel\Target\Php80\Polyfill::substr($block, 0, 32));
        if ($this->counter == 0) {
            $this->counter++;
        }
    }
    ...
    */
    protected function setup()
    {
        ...
        $this->p1 = \Phabel\Target\Php80\Polyfill::substr($constant, 0, 4) . \Phabel\Target\Php80\Polyfill::substr($key, 0, 16) . \Phabel\Target\Php80\Polyfill::substr($constant, 4, 4) . $this->nonce . "\x00\x00\x00\x00";
        $this->p2 = \Phabel\Target\Php80\Polyfill::substr($constant, 8, 4) . \Phabel\Target\Php80\Polyfill::substr($key, 16, 16) . \Phabel\Target\Php80\Polyfill::substr($constant, 12, 4);
    }
    /**
    ...
    */
    public function decrypt($ciphertext)
    {
        if (isset($this->poly1305Key)) {
            ...
            if ($this->oldtag != \Phabel\Target\Php80\Polyfill::substr($newtag, 0, strlen($this->oldtag))) {
                $this->oldtag = false;
    ...
    private function crypt($text, $mode)
    {
        ...
        if (!strlen($buffer['ciphertext'])) {
            ...
            $text = \Phabel\Target\Php80\Polyfill::substr($text, strlen($ciphertext));
            if (!strlen($text)) {
                return $ciphertext;
            }
        }
