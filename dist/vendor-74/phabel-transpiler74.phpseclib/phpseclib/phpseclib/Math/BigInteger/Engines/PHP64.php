...
class PHP64 extends PHP
{
    ...
    const BASE = 31;
    const BASE_FULL = 2147483648;
    const MAX_DIGIT = 2147483647;
    const MSB = 1073741824;
    /**
    ...
    protected function initialize($base)
    {
        ...
        while (true) {
            ...
            if ($i < 0) {
                ...
                $val = \Phabel\Target\Php80\Polyfill::substr($val, 0, 4 + $i);
            ...
            list(, $digit) = unpack('N', \Phabel\Target\Php80\Polyfill::substr($val, $i, 4));
            $step = count($vals) & 7;
            if (!$step) {
