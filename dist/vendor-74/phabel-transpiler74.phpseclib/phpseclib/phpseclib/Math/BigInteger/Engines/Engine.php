...
abstract class Engine implements \JsonSerializable
{
    ...
    public function __construct($x = 0, $base = 10)
    {
        ...
        switch ($base) {
            ...
            case 16:
                if ($base > 0 && $x[0] == '-') {
                    $this->is_negative = true;
                    $x = \Phabel\Target\Php80\Polyfill::substr($x, 1);
                }
    ...
    */
    public function toBits($twos_compliment = false)
    {
        ...
        $result = $this->precision > 0 ? \Phabel\Target\Php80\Polyfill::substr($bits, -$this->precision) : ltrim($bits, '0');
    ...
    */
    ...
    public function bitwise_not()
    {
        ...
        if (strlen($msb) == 8) {
            $msb = \Phabel\Target\Php80\Polyfill::substr($msb, strpos($msb, '0'));
        }
    ...
    protected static function slidingWindow(Engine $x, Engine $e, Engine $n, $class)
    {
        ...
        for ($i = 0; $i < $e_length;) {
            if (!$e_bits[$i]) {
                ...
                $result = static::multiplyReduce($result, $powers[bindec(\Phabel\Target\Php80\Polyfill::substr($e_bits, $i, $j + 1))], $n_value, $class);
                $i += $j + 1;
            }
        }
        $temp = new $class();
