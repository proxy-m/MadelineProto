...
class SSH2
{
    ...
    const MASK_CONNECTED = 2;
    const MASK_LOGIN_REQ = 4;
    const MASK_LOGIN = 8;
    const MASK_SHELL = 16;
    const MASK_WINDOW_ADJUST = 32;
    /*
     * Channel constants
     *
    ...
    */
    ...
    private function connect()
    {
        ...
        if (!is_resource($this->fsock)) {
            ...
            $this->fsock = @\Phabel\Target\Php80\Polyfill::fsockopen($this->host, $this->port, $errno, $errstr, $this->curTimeout == 0 ? 100000 : $this->curTimeout);
            if (!$this->fsock) {
    ...
    private function key_exchange($kexinit_payload_server = false)
    {
        ...
        switch (true) {
            ...
            case \Phabel\Target\Php80\Polyfill::substr($this->server_identifier, 0, 13) == 'SSH-2.0-DLINK':
        ...
        if (strpos($this->kex_algorithm, 'curve25519-sha256') === 0 || strpos($this->kex_algorithm, 'ecdh-sha2-nistp') === 0) {
            $curve = strpos($this->kex_algorithm, 'curve25519-sha256') === 0 ? 'Curve25519' : \Phabel\Target\Php80\Polyfill::substr($this->kex_algorithm, 10);
        ...
        $temp = unpack('Nlength', \Phabel\Target\Php80\Polyfill::substr($this->signature, 0, 4));
        $this->signature_format = \Phabel\Target\Php80\Polyfill::substr($this->signature, 4, $temp['length']);
        $keyBytes = DH::computeSecret($ourPrivate, $theirPublicBytes);
        if (($keyBytes & "\xff\x80") === "\x00\x00") {
            $keyBytes = \Phabel\Target\Php80\Polyfill::substr($keyBytes, 1);
        ...
        if ($this->encrypt) {
            ...
            if ($this->encrypt->usesIV()) {
                ...
                $this->encrypt->setIV(\Phabel\Target\Php80\Polyfill::substr($iv, 0, $this->encrypt_block_size));
            }
            switch ($encrypt) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    $nonce = $kexHash->hash($keyBytes . $this->exchange_hash . 'A' . $this->session_id);
                    $this->encryptFixedPart = \Phabel\Target\Php80\Polyfill::substr($nonce, 0, 4);
                    $this->encryptInvocationCounter = \Phabel\Target\Php80\Polyfill::substr($nonce, 4, 8);
                // fall-through
            ...
            switch ($encrypt) {
                case 'chacha20-poly1305@openssh.com':
                    ...
                    $this->lengthEncrypt->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 32, 32));
            }
            $this->encrypt->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $encryptKeyLength));
        ...
        if ($this->decrypt) {
            ...
            if ($this->decrypt->usesIV()) {
                ...
                $this->decrypt->setIV(\Phabel\Target\Php80\Polyfill::substr($iv, 0, $this->decrypt_block_size));
            }
            switch ($decrypt) {
                case 'aes128-gcm@openssh.com':
                case 'aes256-gcm@openssh.com':
                    ...
                    $this->decryptFixedPart = \Phabel\Target\Php80\Polyfill::substr($nonce, 0, 4);
                    $this->decryptInvocationCounter = \Phabel\Target\Php80\Polyfill::substr($nonce, 4, 8);
                // fall-through
                case 'chacha20-poly1305@openssh.com':
                    break;
                default:
            ...
            switch ($decrypt) {
                case 'chacha20-poly1305@openssh.com':
                    ...
                    $this->lengthDecrypt->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 32, 32));
            }
            $this->decrypt->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $decryptKeyLength));
        ...
        if ($this->hmac_create instanceof Hash) {
            ...
            $this->hmac_create->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $createKeyLength));
            $this->hmac_create_name = $mac_algorithm_out;
        ...
        if ($this->hmac_check instanceof Hash) {
            ...
            $this->hmac_check->setKey(\Phabel\Target\Php80\Polyfill::substr($key, 0, $checkKeyLength));
            $this->hmac_check_name = $mac_algorithm_in;
    ...
    */
    ...
    public function login($username, ...$args)
    {
        ...
        // PuTTY does
        if (\Phabel\Target\Php80\Polyfill::substr($this->server_identifier, 0, 15) != 'SSH-2.0-CoreFTP' && $this->auth_methods_to_continue === null) {
                ...
                if (isset($this->keyboard_requests_responses)) {
                    for ($i = 0; $i < $num_prompts; $i++) {
                        ...
                        foreach ($this->keyboard_requests_responses as $key => $value) {
                            if (\Phabel\Target\Php80\Polyfill::substr($prompt, 0, strlen($key)) == $key) {
                                $responses[] = $value;
                                break;
                            }
                        }
    ...
    * @access public
    */
    public function read($expect = '', $mode = 1)
    {
        $this->curTimeout = $this->timeout;
        $this->is_timeout = false;
        if (!$this->isAuthenticated()) {
        ...
        while (true) {
            if ($mode == self::READ_REGEX) {
                preg_match($expect, \Phabel\Target\Php80\Polyfill::substr($this->interactiveBuffer, -1024), $matches);
        ...
        switch ($this->decompress) {
            ...
            case self::NET_SSH2_COMPRESSION_ZLIB:
                if ($this->regenerate_decompression_context) {
                    ...
                    $payload = \Phabel\Target\Php80\Polyfill::substr($payload, 2);
                }
            ...
            switch (ord($payload[0])) {
                case NET_SSH2_MSG_CHANNEL_REQUEST:
                    if (strlen($payload) == 31) {
                        ...
                        if (\Phabel\Target\Php80\Polyfill::substr($payload, 9, $length) == 'keepalive@openssh.com' && isset($this->server_channels[$channel])) {
                            if (ord(\Phabel\Target\Php80\Polyfill::substr($payload, 9 + $length))) {
                                // want reply
                    ...
                    foreach ($this->channel_buffers[$client_channel] as $i => $packet) {
                        switch (ord($packet[0])) {
                            ...
                            case NET_SSH2_MSG_CHANNEL_FAILURE:
                                ...
                                return \Phabel\Target\Php80\Polyfill::substr($packet, 1);
                        }
                    }
                    break;
                default:
                    return \Phabel\Target\Php80\Polyfill::substr(array_shift($this->channel_buffers[$client_channel]), 1);
            }
        }
        while (true) {
        ...
        if ($this->encrypt) {
            switch ($this->encryptName) {
                ...
                case 'aes256-gcm@openssh.com':
                    ...
                    $packet = $temp . $this->encrypt->encrypt(\Phabel\Target\Php80\Polyfill::substr($packet, 4));
                    break;
                case 'chacha20-poly1305@openssh.com':
                    // This should be impossible, but we are checking anyway to narrow the type for Psalm.
                    ...
                    $packet = $length . $this->encrypt->encrypt(\Phabel\Target\Php80\Polyfill::substr($packet, 4));
                    break;
                default:
                    $packet = $this->hmac_create instanceof Hash && $this->hmac_create_etm ? ($packet & "\xff\xff\xff\xff") . $this->encrypt->encrypt(\Phabel\Target\Php80\Polyfill::substr($packet, 4)) : $this->encrypt->encrypt($packet);
            }
        }
        if ($this->hmac_create instanceof Hash && $this->hmac_create_etm) {
            if (($this->hmac_create->getHash() & "\xff\xff\xff\xff") == 'umac') {
    ...
    protected function format_log($message_log, $message_number_log)
    {
        ...
        for ($i = 0; $i < count($message_log); $i++) {
            ...
            do {
                ...
                $hex = \Phabel\Target\Php80\Polyfill::substr(preg_replace_callback('#.#s', function ($matches) {
