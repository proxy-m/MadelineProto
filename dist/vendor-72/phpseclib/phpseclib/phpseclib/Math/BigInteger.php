...
class BigInteger implements \JsonSerializable
{
     ...
     * @return void
     */
    public static function setEngine($main, array $modexps = array(0 => 'DefaultEngine'))
    {
        self::$engines = [];
    ...
    */
    ...
    {
        self::initialize_static_variables();
        if (\Phabel\Plugin\NewFixer::instanceOf($x, self::$mainEngine)) {
            $this->value = clone $x;
        } elseif ($x instanceof BigInteger\Engines\Engine) {
            $this->value = new static("{$x}");
            $this->value->setPrecision($x->getPrecision());
        } else {
            $this->value = ($phabel_620e3bcd1897fd91 = self::$mainEngine) || true ? new $phabel_620e3bcd1897fd91($x, $base) : false;
        }
    }
    /**
