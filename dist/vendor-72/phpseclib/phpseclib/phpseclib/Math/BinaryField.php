...
class BinaryField extends FiniteField
{
    ...
    public function __construct(...$indices)
    {
        ...
        $h = $bitLen & 7;
        $h = $h ? 8 - $h : 0;
        $r = rtrim(\Phabel\Target\Php80\Polyfill::substr($val, 0, -1), '0');
            ...
            for ($i = $mStart; $i >= $m;) {
                ...
                for (; $mask > 0; $mask >>= 1, $i--, $h++) {
                    if (ord($c[$g]) & $mask) {
                        ...
                        $j = $temp >> 3;
                        $k = $temp & 7;
                        $t1 = $j ? \Phabel\Target\Php80\Polyfill::substr($c, 0, -$j) : $c;
                        $length = strlen($t1);
                        if ($length) {
                            $t2 = str_pad($u[$k], $length, "\x00", STR_PAD_LEFT);
                            $temp = $t1 ^ $t2;
                            $c = $j ? \Phabel\Target\Php80\Polyfill::substr_replace($c, $temp, 0, $length) : $temp;
                        }
                    }
                }
            }
            $c = \Phabel\Target\Php80\Polyfill::substr($c, -$t);
            if (strlen($c) == $t) {
                $c[0] = $c[0] & $finalMask;
            }
            return ltrim($c, "\x00");
