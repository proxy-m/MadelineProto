...
abstract class PHP extends Engine
{
        ...
        switch (abs($base)) {
            ...
            case 10:
                ...
                while (strlen($x)) {
                    ...
                    $temp = $temp->add(new static($this->int2bytes(\Phabel\Target\Php80\Polyfill::substr($x, 0, static::MAX10LEN)), 256));
                    $x = \Phabel\Target\Php80\Polyfill::substr($x, static::MAX10LEN);
                }
                $this->value = $temp->value;
        }
    }
    ...
    public function toBytes($twos_compliment = false)
    {
        ...
        return $this->precision > 0 ? str_pad(\Phabel\Target\Php80\Polyfill::substr($result, -($this->precision + 7 >> 3)), $this->precision + 7 >> 3, chr(0), STR_PAD_LEFT) : $result;
    }
    /**
     * Performs addition.
     *
    ...
    */
    ...
    protected function lshift($shift)
    {
        ...
        while ($num_digits--) {
            \Phabel\Target\Php73\Polyfill::array_unshift($this->value, 0);
        }
    }
    /**
     * Logical Right Shift
