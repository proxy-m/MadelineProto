...
class BCMath extends Engine
{
    ...
    public function toBytes($twos_compliment = false)
    {
        ...
        $current = $this->value;
        if ($current[0] == '-') {
            $current = \Phabel\Target\Php80\Polyfill::substr($current, 1);
        }
        ...
        return $this->precision > 0 ? \Phabel\Target\Php80\Polyfill::substr(str_pad($value, $this->precision >> 3, chr(0), STR_PAD_LEFT), -($this->precision >> 3)) : ltrim($value, chr(0));
    }
    /**
    ...
    */
    public function divide(BCMath $y)
    {
        ...
        if ($remainder->value[0] == '-') {
            $remainder->value = bcadd($remainder->value, $y->value[0] == '-' ? \Phabel\Target\Php80\Polyfill::substr($y->value, 1) : $y->value, 0);
        }
    ...
    */
    ...
    {
        $temp = new static();
        $temp->value = strlen($this->value) && $this->value[0] == '-' ? \Phabel\Target\Php80\Polyfill::substr($this->value, 1) : $this->value;
        return $temp;
    }
    /**
     * Logical And
    ...
    */
    ...
    public function negate()
    {
        ...
        $temp->value = $temp->value[0] == '-' ? \Phabel\Target\Php80\Polyfill::substr($this->value, 1) : '-' . $this->value;
        return $temp;
    }
}