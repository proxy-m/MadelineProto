...
abstract class ASN1
{
     ...
     * @access public
     */
    const ANY_MAP = array(1 => true, 2 => true, 3 => 'bitString', 4 => 'octetString', 5 => 'null', 6 => 'objectIdentifier', 9 => true, 10 => 'enumerated', 12 => 'utf8String', 18 => 'numericString', 19 => 'printableString', 20 => 'teletexString', 21 => 'videotexString', 22 => 'ia5String', 23 => 'utcTime', 24 => 'generalTime', 25 => 'graphicString', 26 => 'visibleString', 27 => 'generalString', 28 => 'universalString', 30 => 'bmpString');
    /**
     ...
     * @access public
     */
    const STRING_TYPE_SIZE = array(12 => 0, 30 => 2, 28 => 4, 19 => 1, 20 => 1, 22 => 1, 26 => 1);
    /**
     * Parse BER-encoding
     *
    ...
    */
    private static function decode_ber($encoded, $start = 0, $encoded_pos = 0)
    {
        ...
        if ($length == 0x80) {
            ...
            $length &= 0x7f;
            $temp = \Phabel\Target\Php80\Polyfill::substr($encoded, $encoded_pos, $length);
            $encoded_pos += $length;
            // tags of indefinte length don't really have a header length; this length includes the tag
            $current += ['headerlength' => $length + 2];
            $start += $length;
            extract(unpack('Nlength', \Phabel\Target\Php80\Polyfill::substr(str_pad($temp, 4, chr(0), STR_PAD_LEFT), -4)));
            /** @var integer $length */
        } else {
        ...
        $content = \Phabel\Target\Php80\Polyfill::substr($encoded, $encoded_pos, $length);
        $content_pos = 0;
        ...
        switch ($class) {
            ...
            case self::CLASS_CONTEXT_SPECIFIC:
                ...
                while ($remainingLength > 0) {
                    ...
                    if (\Phabel\Target\Php80\Polyfill::substr($content, $content_pos + $length, 2) == "\x00\x00") {
                        $length += 2;
                        $start += $length;
                        $newcontent[] = $temp;
                        break;
        ...
        switch ($tag) {
            ...
            case self::TYPE_ENUMERATED:
                ...
                $current['content'] = new BigInteger(\Phabel\Target\Php80\Polyfill::substr($content, $content_pos), -256);
                break;
            case self::TYPE_REAL:
            ...
            case self::TYPE_BIT_STRING:
                ...
                // seven.
                if (!$constructed) {
                    $current['content'] = \Phabel\Target\Php80\Polyfill::substr($content, $content_pos);
                } else {
                    $temp = self::decode_ber($content, $start, $content_pos);
                    if ($temp === false) {
                        return false;
                    ...
                    for ($i = 0; $i < $last; $i++) {
                        ...
                        $current['content'] .= \Phabel\Target\Php80\Polyfill::substr($temp[$i]['content'], 1);
                    }
                    // all subtags should be bit strings
                    if ($temp[$last]['type'] != self::TYPE_BIT_STRING) {
                        return false;
                    }
                    $current['content'] = $temp[$last]['content'][0] . $current['content'] . \Phabel\Target\Php80\Polyfill::substr($temp[$i]['content'], 1);
                }
                break;
            case self::TYPE_OCTET_STRING:
                if (!$constructed) {
                    ...
                    $current['content'] = '';
                    $length = 0;
                    while (\Phabel\Target\Php80\Polyfill::substr($content, $content_pos, 2) != "\x00\x00") {
                    ...
                    if (\Phabel\Target\Php80\Polyfill::substr($content, $content_pos, 2) == "\x00\x00") {
                        $length += 2;
                        // +2 for the EOC
                    }
                }
            ...
            case self::TYPE_SET:
                ...
                while ($content_pos < $content_len) {
                    ...
                    if (!isset($current['headerlength']) && \Phabel\Target\Php80\Polyfill::substr($content, $content_pos, 2) == "\x00\x00") {
                        $length = $offset + 2;
                        // +2 for the EOC
                        break 2;
                    }
            ...
            case self::TYPE_OBJECT_IDENTIFIER:
                ...
                $current['content'] = self::decodeOID(\Phabel\Target\Php80\Polyfill::substr($content, $content_pos));
            ...
            case self::TYPE_GENERALIZED_TIME:
                ...
                $current['content'] = self::decodeTime(\Phabel\Target\Php80\Polyfill::substr($content, $content_pos), $tag);
                break;
            default:
                return false;
        }
     ...
     * @access public
     */
    public static function asn1map($decoded, $mapping, $special = array())
    {
        if (!is_array($decoded)) {
            return false;
        }
        ...
        switch (true) {
            case $mapping['type'] == self::TYPE_ANY:
                ...
                if (isset($decoded['constant']) || !array_key_exists($intype, self::ANY_MAP) || ord(self::$encoded[$decoded['start']]) & 0x20) {
                    return new Element(\Phabel\Target\Php80\Polyfill::substr(self::$encoded, $decoded['start'], $decoded['length']));
                }
    ...
    */
    public static function decodeLength(&$string)
    {
        ...
        if ($length & 0x80) {
            ...
            list(, $length) = unpack('N', \Phabel\Target\Php80\Polyfill::substr(str_pad($temp, 4, chr(0), STR_PAD_LEFT), -4));
        }
        return $length;
    }
    /**
    ...
    * @access public
    */
    public static function encodeDER($source, $mapping, $special = array())
    {
        self::$location = [];
     ...
     * @access private
     */
    private static function encode_der($source, $mapping, $idx = NULL, $special = array())
    {
                    ...
                    if (isset($child['constant'])) {
                        ...
                        */
                        if (isset($child['explicit']) || $child['type'] == self::TYPE_CHOICE) {
                            ...
                            $temp = $subtag . \Phabel\Target\Php80\Polyfill::substr($temp, 1);
                        }
                    }
                    $value .= $temp;
                }
    ...
    public static function decodeOID($content)
    {
        ...
        */
        if ($first <= 2) {
            ...
            \Phabel\Target\Php73\Polyfill::array_unshift($oid, ord($content[0]) % 40);
            \Phabel\Target\Php73\Polyfill::array_unshift($oid, $first);
        } else {
            \Phabel\Target\Php73\Polyfill::array_unshift($oid, $part1->subtract($eighty));
            \Phabel\Target\Php73\Polyfill::array_unshift($oid, 2);
        }
        return implode('.', $oid);
    }
    /**
    ...
    public static function encodeOID($source)
    {
        ...
        \Phabel\Target\Php73\Polyfill::array_unshift($parts, $first->toString());
        $value = '';
    ...
    private static function decodeTime($content, $tag)
    {
        ...
        if ($tag == self::TYPE_UTC_TIME) {
            ...
            $prefix = \Phabel\Target\Php80\Polyfill::substr($content, 0, 2) >= 50 ? '19' : '20';
            $content = $prefix . $content;
        } elseif (strpos($content, '.') !== false) {
            $format .= '.u';
        }
        if ($content[strlen($content) - 1] == 'Z') {
            $content = \Phabel\Target\Php80\Polyfill::substr($content, 0, -1) . '+0000';
        }
    ...
    * @access public
    */
    public static function convert($in, $from = 12, $to = 12)
    {
