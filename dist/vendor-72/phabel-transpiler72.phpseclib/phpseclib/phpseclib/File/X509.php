...
class X509
{
     ...
     * @return mixed
     */
    public function loadX509($cert, $mode = 3)
    {
        ...
        $this->signatureSubject = \Phabel\Target\Php80\Polyfill::substr($cert, $decoded[0]['content'][0]['start'], $decoded[0]['content'][0]['length']);
     ...
     * @return string
     */
    public function saveX509($cert, $format = 0)
    {
        if (!is_array($cert) || !isset($cert['tbsCertificate'])) {
            return false;
        }
    ...
    */
    ...
    private static function fetchURL($url)
    {
        ...
        switch ($parts['scheme']) {
            case 'http':
                $fsock = @\Phabel\Target\Php80\Polyfill::fsockopen($parts['host'], isset($parts['port']) ? $parts['port'] : 80);
                if (!$fsock) {
                    return false;
                }
    ...
    private function validateSignatureCountable($caonly, $count)
    {
        ...
        switch (true) {
            case isset($this->currentCert['tbsCertificate']):
                ...
                return $this->validateSignatureHelper($signingCert['tbsCertificate']['subjectPublicKeyInfo']['algorithm']['algorithm'], $signingCert['tbsCertificate']['subjectPublicKeyInfo']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], \Phabel\Target\Php80\Polyfill::substr($this->currentCert['signature'], 1), $this->signatureSubject);
            case isset($this->currentCert['certificationRequestInfo']):
                return $this->validateSignatureHelper($this->currentCert['certificationRequestInfo']['subjectPKInfo']['algorithm']['algorithm'], $this->currentCert['certificationRequestInfo']['subjectPKInfo']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], \Phabel\Target\Php80\Polyfill::substr($this->currentCert['signature'], 1), $this->signatureSubject);
            case isset($this->currentCert['publicKeyAndChallenge']):
                return $this->validateSignatureHelper($this->currentCert['publicKeyAndChallenge']['spki']['algorithm']['algorithm'], $this->currentCert['publicKeyAndChallenge']['spki']['subjectPublicKey'], $this->currentCert['signatureAlgorithm']['algorithm'], \Phabel\Target\Php80\Polyfill::substr($this->currentCert['signature'], 1), $this->signatureSubject);
    ...
    */
    ...
    {
        $size = strlen($ip) >> 1;
        $mask = \Phabel\Target\Php80\Polyfill::substr($ip, $size);
        $ip = \Phabel\Target\Php80\Polyfill::substr($ip, 0, $size);
    ...
    public function removeDNProp($propName)
    {
        ...
        if (!isset($dn[0])) {
            $dn = \Phabel\Target\Php74\Polyfill::array_splice($dn, 0, 0);
        }
    }
    /**
     ...
     */
    public function getDN($format = 0, $dn = NULL)
    {
        if (!isset($dn)) {
     ...
     * @return mixed
     */
    public function getIssuerDN($format = 0)
    {
        switch (true) {
    ...
    * @return mixed
    */
    public function getSubjectDN($format = 0)
    {
        switch (true) {
            case !empty($this->dn):
     ...
     * @access public
     */
    public function loadCSR($csr, $mode = 3)
    {
        ...
        $this->dn = $csr['certificationRequestInfo']['subject'];
        $this->signatureSubject = \Phabel\Target\Php80\Polyfill::substr($orig, $decoded[0]['content'][0]['start'], $decoded[0]['content'][0]['length']);
     ...
     * @return string
     */
    public function saveCSR($csr, $format = 0)
    {
        if (!is_array($csr) || !isset($csr['certificationRequestInfo'])) {
            return false;
        }
     ...
     * @return string
     */
    public function saveSPKAC($spkac, $format = 0)
    {
        if (!is_array($spkac) || !isset($spkac['publicKeyAndChallenge'])) {
            return false;
        }
     ...
     * @access public
     */
    public function loadCRL($crl, $mode = 3)
    {
     ...
     * @return string
     */
    public function saveCRL($crl, $format = 0)
    {
        if (!is_array($crl) || !isset($crl['tbsCertList'])) {
            return false;
        }
    ...
    */
    ...
    private function removeExtensionHelper($id, $path = NULL)
    {
        ...
        if (!isset($extensions[0])) {
            $extensions = \Phabel\Target\Php74\Polyfill::array_splice($extensions, 0, 0);
        }
        return $result;
    }
    /**
     ...
     * @return bool
     */
    public function removeAttribute($id, $disposition = -1)
    {
     ...
     * @return mixed
     */
    public function getAttribute($id, $disposition = -1, $csr = NULL)
    {
        if (empty($csr)) {
     ...
     * @return bool
     */
    public function setAttribute($id, $value, $disposition = -1)
    {
    ...
    */
    public function computeKeyIdentifier($key = NULL, $method = 1)
    {
        ...
        $hash = $hash->hash($key);
        if ($method == 2) {
            $hash = \Phabel\Target\Php80\Polyfill::substr($hash, -8);
