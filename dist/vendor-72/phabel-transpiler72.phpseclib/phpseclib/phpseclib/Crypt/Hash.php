...
class Hash
{
    ...
    */
    ...
    public function setHash($hash)
    {
        ...
        switch ($hash) {
            ...
            case 'umac-128':
                $this->blockSize = 128;
                $this->length = abs(\Phabel\Target\Php80\Polyfill::substr($hash, -3)) >> 3;
                $this->algo = 'umac';
                return;
            case 'md2-96':
            case 'md5-96':
            ...
            case 'sha512/224-96':
            case 'sha512/256-96':
                $hash = \Phabel\Target\Php80\Polyfill::substr($hash, 0, -3);
                $this->length = 12;
                // 96 / 8 = 12
                break;
            case 'md2':
        ...
        if (in_array(\Phabel\Target\Php80\Polyfill::substr($hash, 0, 5), ['sha3-', 'shake', 'kecca'])) {
            // PHP 7.1.0 introduced support for "SHA3 fixed mode algorithms":
            // http://php.net/ChangeLog-7.php#7.1.0
            if (version_compare(PHP_VERSION, '7.1.0') < 0 || \Phabel\Target\Php80\Polyfill::substr($hash, 0, 5) != 'sha3-') {
    ...
    private function pdf()
    {
        ...
        return $taglen <= 8 ? \Phabel\Target\Php80\Polyfill::substr($t, unpack('C', $index)[1] * $taglen, $taglen) : \Phabel\Target\Php80\Polyfill::substr($t, 0, $taglen);
    }
    /**
     * UHASH Algorithm
     *
    ...
    private function uhash($m, $taglen)
    {
        ...
        $y = '';
        for ($i = 0; $i < $iters; $i++) {
            $L1Key_i = \Phabel\Target\Php80\Polyfill::substr($L1Key, $i * 16, 1024);
            $L2Key_i = \Phabel\Target\Php80\Polyfill::substr($L2Key, $i * 24, 24);
            $L3Key1_i = \Phabel\Target\Php80\Polyfill::substr($L3Key1, $i * 64, 64);
            $L3Key2_i = \Phabel\Target\Php80\Polyfill::substr($L3Key2, $i * 4, 4);
    ...
    */
    private static function L2Hash($k, $m)
    {
        ...
        $k128 = \Phabel\Target\Php80\Polyfill::substr($k, 8) & "\x01\xff\xff\xff\x01\xff\xff\xff\x01\xff\xff\xff\x01\xff\xff\xff";
        ...
        if (strlen($m) <= 0x20000) {
            ...
            $m_1 = \Phabel\Target\Php80\Polyfill::substr($m, 0, 0x20000);
            // 1 << 17
            $m_2 = \Phabel\Target\Php80\Polyfill::substr($m, 0x20000) . "\x80";
            $length = strlen($m_2);
            $pad = 16 - $length % 16;
            $pad %= 16;
    ...
    */
    private static function L3Hash($k1, $k2, $m)
    {
        ...
        for ($i = 0; $i < 8; $i++) {
            $m_i = $factory->newInteger(new BigInteger(\Phabel\Target\Php80\Polyfill::substr($m, 2 * $i, 2), 256));
            $k_i = $factory->newInteger(new BigInteger(\Phabel\Target\Php80\Polyfill::substr($k1, 8 * $i, 8), 256));
            $y = $y->add($m_i->multiply($k_i));
        }
        $y = str_pad(\Phabel\Target\Php80\Polyfill::substr($y->toBytes(), -4), 4, "\x00", STR_PAD_LEFT);
        $y = $y ^ $k2;
        return $y;
    }
    /**
    ...
    public function hash($text)
    {
        ...
        if (is_array($algo)) {
            if (empty($this->key) || !is_string($this->key)) {
                return \Phabel\Target\Php80\Polyfill::substr($algo($text, ...array_values($this->parameters)), 0, $this->length);
            }
            ...
            $temp = $this->ipad ^ $key;
            $temp .= $text;
            $temp = \Phabel\Target\Php80\Polyfill::substr($algo($temp, ...array_values($this->parameters)), 0, $this->length);
            $output = $this->opad ^ $key;
            $output .= $temp;
            $output = $algo($output, ...array_values($this->parameters));
            return \Phabel\Target\Php80\Polyfill::substr($output, 0, $this->length);
        }
        $output = !empty($this->key) || is_string($this->key) ? hash_hmac($algo, $text, $this->computedKey, true) : hash($algo, $text, true);
        return strlen($output) > $this->length ? \Phabel\Target\Php80\Polyfill::substr($output, 0, $this->length) : $output;
    }
    /**
