...
abstract class PKCS8 extends PKCS
{
    ...
    {
        $algo = preg_match('#^pbeWith(?:MD2|MD5|SHA1|SHA)And(.*?)-CBC$#', $algo, $matches) ? $matches[1] : \Phabel\Target\Php80\Polyfill::substr($algo, 13);
    ...
    private static function getPBES2EncryptionObject($algo)
    {
        switch ($algo) {
            ...
            case 'aes256-CBC-PAD':
                $cipher = new AES('cbc');
                $cipher->setKeyLength(\Phabel\Target\Php80\Polyfill::substr($algo, 3, 3));
                break;
            default:
    ...
    protected static function load($key, $password = '')
    {
                ...
                case 'id-PBES2':
                    ...
                    switch ($keyDerivationFunc['algorithm']) {
                        case 'id-PBKDF2':
                            ...
                            $hash = str_replace('-', '/', \Phabel\Target\Php80\Polyfill::substr($prf['algorithm'], 11));
        ...
        if (is_array($private)) {
            if (isset($private['privateKeyAlgorithm']['parameters']) && !$private['privateKeyAlgorithm']['parameters'] instanceof ASN1\Element && isset($decoded[0]['content'][1]['content'][1])) {
                ...
                $private['privateKeyAlgorithm']['parameters'] = new ASN1\Element(\Phabel\Target\Php80\Polyfill::substr($key, $temp['start'], $temp['length']));
            }
            ...
            if (isset($private['publicKey'])) {
                ...
                $private['publicKey'] = \Phabel\Target\Php80\Polyfill::substr($private['publicKey'], 1);
            }
            return $private + $meta;
        }
        ...
        if (is_array($public)) {
            ...
            if (isset($public['publicKeyAlgorithm']['parameters']) && !$public['publicKeyAlgorithm']['parameters'] instanceof ASN1\Element && isset($decoded[0]['content'][0]['content'][1])) {
                ...
                $public['publicKeyAlgorithm']['parameters'] = new ASN1\Element(\Phabel\Target\Php80\Polyfill::substr($key, $temp['start'], $temp['length']));
            }
            $public['publicKey'] = \Phabel\Target\Php80\Polyfill::substr($public['publicKey'], 1);
            return $public;
        }
     ...
     * @return string
     */
    protected static function wrapPrivateKey($key, $attr, $params, $password, $oid = NULL, $publicKey = '', array $options = array())
    {
        ...
        if (!empty($password) && is_string($password)) {
            ...
            if ($encryptionAlgorithm == 'id-PBES2') {
                ...
                $hash = str_replace('-', '/', \Phabel\Target\Php80\Polyfill::substr($prf, 11));
                $kdf = 'pbkdf2';
