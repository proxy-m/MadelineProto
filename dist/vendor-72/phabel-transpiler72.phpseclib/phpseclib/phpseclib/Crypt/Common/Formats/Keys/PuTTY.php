...
abstract class PuTTY
{
    ...
    private static function generateV2Key($password, $length)
    {
        ...
        return \Phabel\Target\Php80\Polyfill::substr($symkey, 0, $length);
    }
    /**
    ...
    private static function generateV3Key($password, $flavour, $memory, $passes, $salt)
    {
        ...
        $symkey = \Phabel\Target\Php80\Polyfill::substr($temp, 0, 32);
        $symiv = \Phabel\Target\Php80\Polyfill::substr($temp, 32, 16);
        $hashkey = \Phabel\Target\Php80\Polyfill::substr($temp, -32);
    ...
    public static function load($key, $password)
    {
        ...
        if (strpos($key, 'BEGIN SSH2 PUBLIC KEY') !== false) {
            ...
            $lines = \Phabel\Target\Php74\Polyfill::array_splice($lines, 1, -1);
            ...
            foreach ($lines as $line) {
                switch (true) {
                    case preg_match('#^(.*?): (.*)#', $line, $match):
                        ...
                        $values[$current] = $in_value ? \Phabel\Target\Php80\Polyfill::substr($match[2], 0, -1) : $match[2];
                        break;
                    case $in_value:
                        $in_value = $line[strlen($line) - 1] == '\\';
                        $values[$current] .= $in_value ? \Phabel\Target\Php80\Polyfill::substr($line, 0, -1) : $line;
                        break;
                    default:
                        $data .= $line;
                }
