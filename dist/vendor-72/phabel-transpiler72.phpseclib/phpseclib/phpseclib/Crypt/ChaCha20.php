...
class ChaCha20 extends Salsa20
{
    ...
    */
    ...
    private function encrypt_with_libsodium($plaintext)
    {
        ...
        if (!$this->usePoly1305) {
            return \Phabel\Target\Php80\Polyfill::substr($ciphertext, 0, strlen($plaintext));
        }
        $newciphertext = \Phabel\Target\Php80\Polyfill::substr($ciphertext, 0, strlen($plaintext));
        $this->newtag = $this->usingGeneratedPoly1305Key && strlen($this->nonce) == 12 ? \Phabel\Target\Php80\Polyfill::substr($ciphertext, strlen($plaintext)) : $this->poly1305($newciphertext);
    ...
    private function decrypt_with_libsodium($ciphertext)
    {
        ...
        return \Phabel\Target\Php80\Polyfill::substr($plaintext, 0, strlen($ciphertext));
    }
    /**
     * Sets the nonce.
     *
