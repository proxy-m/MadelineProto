...
exec sftp-server';
        ...
        */
        ...
        if (isset($this->extensions['versions']) && (!$this->preferredVersion || $this->preferredVersion != $this->version)) {
            ...
            if ($this->preferredVersion) {
                ...
                \Phabel\Target\Php73\Polyfill::array_unshift($supported, $this->preferredVersion);
            }
    ...
    public function chdir($dir)
    {
        ...
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
                $handle = \Phabel\Target\Php80\Polyfill::substr($response, 4);
                break;
            case NET_SFTP_STATUS:
                $this->logError($response);
                return false;
            default:
                throw new \UnexpectedValueException('Expected NET_SFTP_HANDLE or NET_SFTP_STATUSGot packet type: ' . $this->packet_type);
        }
    ...
    */
    ...
    public function touch($filename, $time = NULL, $atime = NULL)
    {
        ...
        switch ($this->packet_type) {
            case NET_SFTP_HANDLE:
                return $this->close_handle(\Phabel\Target\Php80\Polyfill::substr($response, 4));
            case NET_SFTP_STATUS:
                $this->logError($response);
                break;
            default:
    ...
    */
    ...
    private function setstat($filename, $attr, $recursive)
    {
        ...
        $packet = Strings::packSSH2('s', $filename);
        $packet .= $this->version >= 4 ? pack('a*Ca*', \Phabel\Target\Php80\Polyfill::substr($attr, 0, 4), NET_SFTP_TYPE_UNKNOWN, \Phabel\Target\Php80\Polyfill::substr($attr, 4)) : $attr;
        ...
        if ($this->version == 6) {
            ...
            /* quoting http://bxr.su/OpenBSD/usr.bin/ssh/PROTOCOL#347 :
                              ...
                              SSH_FXP_SYMLINK as follows:
                                  ...
                                  string      targetpath
                                  string      linkpath */
            $packet = \Phabel\Target\Php80\Polyfill::substr($this->server_identifier, 0, 15) == 'SSH-2.0-OpenSSH' ? Strings::packSSH2('ss', $target, $link) : Strings::packSSH2('ss', $link, $target);
        }
        $this->send_sftp_packet($type, $packet);
        $response = $this->get_sftp_packet();
        if ($this->packet_type != NET_SFTP_STATUS) {
    ...
    * @access public
    */
    public function put($remote_file, $data, $mode = 2, $start = -1, $local_start = -1, $progressCallback = NULL)
    {
        if (!$this->precheck()) {
            return false;
        }
        ...
        while ($dataCallback || ($size === 0 || $sent < $size)) {
            if ($dataCallback) {
                ...
                $temp = isset($fp) ? fread($fp, $sftp_packet_size) : \Phabel\Target\Php80\Polyfill::substr($data, $sent, $sftp_packet_size);
                if ($temp === false || $temp === '') {
                    break;
                }
            }
    ...
    public function get($remote_file, $local_file = false, $offset = 0, $length = -1, $progressCallback = NULL)
    {
        ...
        while (true) {
            ...
            while ($i > 0) {
                ...
                switch ($this->packet_type) {
                    case NET_SFTP_DATA:
                        $temp = \Phabel\Target\Php80\Polyfill::substr($response, 4);
                        $offset += strlen($temp);
                        if ($local_file === false) {
                            $content .= $temp;
        ...
        if ($length > 0 && $length <= $offset - $start) {
            if ($local_file === false) {
                $content = \Phabel\Target\Php80\Polyfill::substr($content, 0, $length);
            } else {
