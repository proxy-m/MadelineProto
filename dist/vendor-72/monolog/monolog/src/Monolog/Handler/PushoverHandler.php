...
class PushoverHandler extends SocketHandler
{
    ...
    * @param bool $bubble
    */
    public function __construct($token, $users, $title = NULL, $level = 500, $bubble = true, bool $useSSL = true, $highPriorityLevel = 500, $emergencyLevel = 600, int $retry = 30, int $expire = 25200, bool $persistent = false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = NULL, ?int $chunkSize = NULL)
    {
        if (!\is_string($token)) {
        ...
        if (!(\is_string($title) || \is_null($title))) {
            if (!(\is_string($title) || \is_object($title) && \method_exists($title, '__toString') || (\is_bool($title) || \is_numeric($title)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($title) must be of type ?string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($title) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $title = (string) $title;
            }
        }
