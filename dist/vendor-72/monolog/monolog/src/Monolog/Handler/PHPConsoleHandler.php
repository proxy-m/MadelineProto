...
class PHPConsoleHandler extends AbstractProcessingHandler
{
     ...
     */
    public function __construct($options = array(), $connector = NULL, $level = 100, bool $bubble = true)
    {
        if (!\is_array($options)) {
        ...
        if (!($connector instanceof Connector || \is_null($connector))) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($connector) must be of type ?Connector, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($connector) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . Utils::jsonEncode($this->connector->getDumper()->dump(\Phabel\Target\Php74\Polyfill::array_filter($record['context'])), null, true);
        }
