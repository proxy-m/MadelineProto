...
class NativeMailerHandler extends MailHandler
{
     ...
     */
    public function __construct($to, $subject, string $from, $level = 400, bool $bubble = true, int $maxColumnWidth = 70)
    {
        if (!\is_string($subject)) {
            if (!(\is_string($subject) || \is_object($subject) && \method_exists($subject, '__toString') || (\is_bool($subject) || \is_numeric($subject)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($subject) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($subject) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
