...
class SlackHandler extends SocketHandler
{
    ...
    public function __construct($token, $channel, $username = NULL, bool $useAttachment = true, $iconEmoji = NULL, $level = 500, bool $bubble = true, bool $useShortAttachment = false, bool $includeContextAndExtra = false, array $excludeFields = array(), bool $persistent = false, float $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = NULL, ?int $chunkSize = NULL)
    {
        ...
        if (!(\is_string($username) || \is_null($username))) {
            if (!(\is_string($username) || \is_object($username) && \method_exists($username, '__toString') || (\is_bool($username) || \is_numeric($username)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #3 ($username) must be of type ?string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($username) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $username = (string) $username;
            }
        }
        if (!(\is_string($iconEmoji) || \is_null($iconEmoji))) {
            if (!(\is_string($iconEmoji) || \is_object($iconEmoji) && \method_exists($iconEmoji, '__toString') || (\is_bool($iconEmoji) || \is_numeric($iconEmoji)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #5 ($iconEmoji) must be of type ?string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($iconEmoji) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
