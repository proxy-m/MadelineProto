...
class SyslogUdpHandler extends AbstractSyslogHandler
{
     ...
     */
    public function __construct($host, $port = 514, $facility = LOG_USER, $level = 100, bool $bubble = true, string $ident = 'php', int $rfc = 1)
    {
        if (!\is_string($host)) {
            if (!(\is_string($host) || \is_object($host) && \method_exists($host, '__toString') || (\is_bool($host) || \is_numeric($host)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($host) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($host) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $host = (string) $host;
            }
        }
