...
class TL
{
     ...
     * @return void
     */
    public function init(TLSchema $files, array $objects = array())
    {
        ...
        foreach (\Phabel\Target\Php74\Polyfill::array_filter(\array_merge(array('api' => $files->getAPISchema(), 'mtproto' => $files->getMTProtoSchema(), 'secret' => $files->getSecretSchema()), $files->getOther())) as $scheme_type => $file) {
    ...
    private function serializeParams(array $tl, $arguments, $ctx, int $layer, $promise) : \Generator
    {
        ...
        foreach ($tl['params'] as $cur_flag) {
            if (isset($cur_flag['pow'])) {
                $arguments[$cur_flag['flag']] = $arguments[$cur_flag['flag']] ?? 0;
     ...
     * @return int
     */
    public function getLength($stream, $type = array('type' => '')) : int
    {
        if (\is_string($stream)) {
     ...
     */
    public function deserialize($stream, $type = array('type' => '')) : array
    {
        $promises = [];
