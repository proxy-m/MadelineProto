...
final class Entities
{
    ...
    * @param mixed $end
    */
    private function setOffset(string $start, $end = '</a>') : void
    {
        if (!true) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($end) must be of type mixed, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($end) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
    ...
    */
    private function setEntitie(int $type, array $array = array(), string $intag = '', &$i = 0)
    {
        $result = ["_" => "", "offset" => $this->offset, "length" => 0];
        if ($type == ATAG) {
            if (isset($array["href"])) {
                if (\preg_match('/^(?:tg:\\/\\/user\\?id=|mention:)(.*)$/isu', $array["href"], $matches)) {
                    $userId = $matches[1];
                    if (!\is_numeric($userId)) {
                        try {
                            $userId = $userId ?? $this->api->getInfo($matches[1])['id'];
                        } catch (\Throwable $e) {
                        }
                    }
                ...
                $phabelReturn = null;
                if (!(\is_array($phabelReturn) || \is_null($phabelReturn) || \is_bool($phabelReturn))) {
                    if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                        throw new \TypeError(__METHOD__ . '(): Return value must be of type array|null|bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                    } else {
            ...
            if ($intag === "pre" && (isset($array["class"]) && \preg_match('/^language\\-(.*?)$/', $array["class"], $matches) || isset($array["language"]))) {
                $result["_"] = $this->getEntityName(PRE);
                $this->entities[$this->entitiesid - 1]["language"] = $matches[1];
                $phabelReturn = null;
                if (!(\is_array($phabelReturn) || \is_null($phabelReturn) || \is_bool($phabelReturn))) {
                    if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                        throw new \TypeError(__METHOD__ . '(): Return value must be of type array|null|bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
    ...
    * @return array
    */
    private function checkEntity($entity, &$type) : array
    {
        if (!(\is_object($entity) || \is_array($entity))) {
            throw new \TypeError(__METHOD__ . '(): Argument #1 ($entity) must be of type object|array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($entity) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        if (\is_object($entity)) {
            $entity = (array) $entity;
        }
        if (\is_array($entity)) {
            ...
            if (\is_array($entity["offset"]) || \is_object($entity["offset"]) || (string) (int) $entity["offset"] !== (string) $entity["offset"]) {
                throw new Exception('Field "offset" must be of type Integer');
            }
            if (\is_array($entity["length"]) || \is_object($entity["length"]) || (string) (int) $entity["length"] !== (string) $entity["length"]) {
    ...
    * @return string
    */
    public function entitiesToHtml(string $text, $entities = array(), bool $specialchars = true) : string
    {
        if (!(\is_array($entities) || \is_object($entities))) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($entities) must be of type object|array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($entities) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        $this->setOffset = [];
        $this->setOffset2 = [];
        ...
        foreach ($entities as $entity) {
            ...
            if ($type == CODE) {
                ...
                $this->setOffset('<a href="tg://user?id=' . ($entity["user"]["id"] ?? $entity["user_id"] ?? 0) . '">');
            } elseif ($type == PRE) {
                if (isset($entity["language"])) {
                    $this->setOffset('<pre><code class="language-' . $entity["language"] . '">', "</code></pre>");
                } else {
        ...
        foreach ($this->setOffset2 as $key => $value) {
            ...
            $this->setOffset[$key] = \array_merge(\array_reverse($this->setOffset2[$key]), $this->setOffset[$key]);
        }
        $htmlext = "";
        $deltag = ['& a m p ; ', '& l t ; ', '& g t ; '];
        $deltag2 = ['/&\\000/', '/\\<\\000/', '/\\>\\000/'];
        for ($offset = 0; $offset < \strlen($utf16) / 2; $offset++) {
            $t = \Phabel\Target\Php80\Polyfill::substr($utf16, $offset * 2, 2);
    ...
    * @return string
    */
    public function entitiesToMarkdownV1(string $text, $entities = array(), bool $slashmarkdown = true) : string
    {
        ...
        foreach ($entities as $entity) {
            ...
            if ($type == CODE) {
                ...
                $this->setOffset("[", "](tg://user?id=" . ($entity["user"]["id"] ?? $entity["user_id"] ?? 0) . ")");
            } elseif ($type == PRE) {
                if (isset($entity["language"])) {
    ...
    * @return string
    */
    public function entitiesToMarkdown(string $text, $entities = array(), bool $slashmarkdown = true) : string
    {
        if (!(\is_object($entities) || \is_array($entities))) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($entities) must be of type object|array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($entities) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        ...
        $htmlext = "";
        $deltag = ['\\ _ ', '\\ * ', '\\ [ ', '\\ ] ', '\\ ( ', '\\ ) ', '\\ ~ ', '\\  ', '\\ > ', '\\ # ', '\\ + ', '\\ - ', '\\ = ', '\\ | ', '\\ { ', '\\ } ', '\\ . ', '\\ ! '];
        $deltag2 = ['/_\\000/', '/\\*\\000/', '/\\[\\000/', '/\\]\\000/', '/\\(\\000/', '/\\)\\000/', '/~\\000/', '/`\\000/', '/\\>\\000/', '/\\#\\000/', '/\\+\\000/', '/\\-\\000/', '/\\=\\000/', '/\\|\\000/', '/\\{\\000/', '/\\}\\000/', '/\\.\\000/', '/\\!\\000/'];
        for ($offset = 0; $offset < \strlen($utf16) / 2; $offset++) {
            $t = \Phabel\Target\Php80\Polyfill::substr($utf16, $offset * 2, 2);
            if (isset($this->setOffset[$offset])) {
                foreach ($this->setOffset[$offset] as $tt) {
    ...
    public function markdownV1ToHtml(string $str, bool $specialchars = true) : string
    {
        ...
        $find = function ($str, $find, &$i) use($backslash) {
            ...
            for ($i = 0; $i < \mb_strlen($str); $i++) {
                ...
                if ($curchar == "\\" && \in_array(\mb_substr($str, $i + 1, 1), $backslash)) {
        ...
        $i = 0;
        $setmark = function ($mark, &$currentmarki = 0, $fakemark = false) use(&$marks, &$marksi, &$htmli, &$i) {
            if ($marksi === -1 || $marks[$marksi][0] !== $mark) {
                $marksi++;
                $marks[$marksi] = [$fakemark == false ? $mark : $fakemark, $htmli, $i];
                return true;
            }
        ...
        $currentmarki = 0;
        $setstr2 = function ($endtag, $starttaglen) use(&$setstr, &$html, &$htmli, &$currentmarki) {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($curchar == "\\" && \in_array(\mb_substr($str, $i + 1, 1), $backslash)) {
                ...
                if (!$setmark("]", $currentmarki, false) && $is("](")) {
                    $txt = \mb_substr($html, $currentmarki, $htmli - $currentmarki);
                    if ($txt !== "") {
                        $i++;
                        $strfind = $find(\mb_substr($str, $i + 1), ")", $pos);
                        if ($strfind !== false) {
                            $i += $pos + 1;
                            $html = \mb_substr($html, 0, $currentmarki) . '<a href="' . $strfind . '">' . $txt . "</a>";
                    ...
                    if ($strfind !== false) {
                        ...
                        if ($strfind !== "") {
                            ...
                            if ($lang) {
                                $strfind = \trim($f);
                                if ($strfind !== "") {
                                    $setstr('<pre><code class="language-' . $lang . '">' . $strfind . "</code></pre>");
                                }
                            } else {
                        ...
                        throw new Exception('Can\'t find end of Pre entity at byte offset ' . $i);
                    }
                } else {
                    $strfind = $find(\mb_substr($str, $i + 1), "`", $pos);
                    if ($strfind !== false) {
                        ...
                        throw new Exception('Can\'t find end of Code entity at byte offset ' . $i);
                    }
                }
            } elseif (\in_array($curchar, $backslash)) {
                throw new Exception("Character '{$curchar}' is reserved and must be escaped with the preceding '\\'");
            } else {
                $setstr($curchar);
            }
        }
        foreach ($marks as $mark) {
            $ar = ["*" => "Bold", "_" => "Italic", "]" => "TextUrl"];
            throw new Exception('Can\'t find end of ' . ($ar[$mark[0]] ?? $mark[0]) . " entity at byte offset " . $mark[2]);
        }
        return $html;
    }
    /**
    ...
    public function markdownToHtml(string $str, bool $specialchars = true) : string
    {
        ...
        for ($i = 0; $i < $len; $i++) {
            ...
            if ($curchar == "\\" && \in_array(\mb_substr($str, $i + 1, 1), $backslash)) {
                ...
                $tag = "i";
                if (($marksi === -1 || $marks[$marksi][0] !== $curchar) && $is("**")) {
                    $curchar = "**";
                    $tag = "b";
                    $i++;
                }
                ...
                $tag = "u";
                if (($marksi === -1 || $marks[$marksi][0] !== $curchar) && $is("__")) {
                    $curchar = "__";
                    $tag = "i";
                    $i++;
                }
        ...
        foreach ($marks as $mark) {
            $ar = ["**" => "Bold", "__" => "Italic", "*" => "Italic", "_" => "Underline", "~~" => "Strikethrough", "]" => "TextUrl", "||" => "Spoiler"];
            throw new Exception('Can\'t find end of ' . ($ar[$mark[0]] ?? $mark[0]) . " entity at byte offset " . $mark[2]);
        }
        return $html;
    }
    ...
    */
    ...
    {
        return $this->htmlToEntities($this->markdownToHtml($text));
    }
    /**
    ...
    */
    ...
    {
        return $this->htmlToEntities($this->markdownV1ToHtml($text));
    }
    /**
    ...
    */
    ...
    {
        return $this->htmlToEntities($this->markdownToHtml($text, false));
    }
    /**
    ...
    */
    ...
    {
        return $this->htmlToEntities($this->markdownV1ToHtml($text, false));
    }
    /**
    * htmlToMarkdown
    convert html tags to markdown format.
    ...
    */
    ...
    {
        return (function ($phabel_daf6257ff145c1f9) use(&$mode, &$text) {
            if ($phabel_daf6257ff145c1f9 === 'html') {
                return $this->htmlToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdown') {
                return $this->markdownToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdownv2') {
                return $this->markdownToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdownv1') {
                return $this->markdownV1ToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdownhtml') {
                return $this->markdownhtmlToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdownv2html') {
                return $this->markdownhtmlToEntities($text);
            } elseif ($phabel_daf6257ff145c1f9 === 'markdownv1html') {
                return $this->markdownV1htmlToEntities($text);
            } else {
                return \Phabel\Target\Php80\ThrowExprReplacer::throwMe(new Exception("unsupported mode"));
            }
        })($mode);
    }
}