...
class ReferenceDatabase implements TLCallback
{
    ...
    const LOCATION_CONTEXT = array('inputDocumentFileLocation' => 0, 'inputPhotoFileLocation' => 1, 'inputPhoto' => 1, 'inputDocument' => 0);
    const METHOD_CONTEXT = array('photos.updateProfilePhoto' => 0, 'photos.getUserPhotos' => 0, 'photos.uploadProfilePhoto' => 0, 'messages.getStickers' => 8);
    const CONSTRUCTOR_CONTEXT = array('message' => 3, 'messageService' => 3, 'chatFull' => 1, 'channelFull' => 1, 'chat' => 1, 'channel' => 1, 'updateUserPhoto' => 0, 'user' => 0, 'userFull' => 0, 'wallPaper' => 9, 'messages.savedGifs' => 4, 'messages.recentStickers' => 5, 'messages.favedStickers' => 6, 'messages.stickerSet' => 2, 'document' => 2);
    /**
     ...
     * @var array
     */
    protected static $dbProperties = ['db' => 'array'];
    /**
     *
     */
    ...
    public function init() : \Generator
    {
        ...
        Loop::defer(function () {
            return (yield $this->db->clear());
        });
        //Clear table every day
        Loop::repeat(24 * 1000 * 3600, function () {
            return (yield $this->db->clear());
        });
    }
    /**
     ...
     *
     */
    public function addOrigin(array $data = array()) : \Generator
    {
        $key = \count($this->cacheContexts) - 1;
        if ($key === -1) {
