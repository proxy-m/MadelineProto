...
{
    private const REPS = array(0 => 'succeeded', 1 => 'general SOCKS server failure', 2 => 'connection not allowed by ruleset', 3 => 'Network unreachable', 4 => 'Host unreachable', 5 => 'Connection refused', 6 => 'TTL expired', 7 => 'Command not supported', 8 => 'Address type not supported');
    use RawStream;
    /**
     * Stream.
     *
    ...
    {
        $phabelReturn = $this->stream->getSocket();
        if (!$phabelReturn instanceof EncryptableSocket) {
            throw new \TypeError(__METHOD__ . '(): Return value must be of type EncryptableSocket, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        return $phabelReturn;
    }
    /**
