...
class Exception extends \Exception
{
     ...
     */
    public function __construct($message = NULL, $code = 0, $previous = NULL, $file = NULL, $line = NULL)
    {
        if (!($previous instanceof self || \is_null($previous))) {
            throw new \TypeError(__METHOD__ . '(): Argument #3 ($previous) must be of type ?' . self::class . ', ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($previous) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
        $this->prettifyTL();
        if ($file !== null) {
            $this->file = $file;
