...
class Parsedown
{
    ...
    protected function lines(array $lines)
    {
        ...
        foreach ($lines as $line) {
            ...
            $text = $indent > 0 ? \Phabel\Target\Php80\Polyfill::substr($line, $indent) : $line;
            # ~
    ...
    protected function blockCode($Line, $Block = NULL)
    {
        ...
        if ($Line['indent'] >= 4) {
            $text = \Phabel\Target\Php80\Polyfill::substr($Line['body'], 4);
    ...
    protected function blockFencedCode($Line)
    {
        if (preg_match('/^[' . $Line['text'][0] . ']{3,}[ ]*([^`]+)?[ ]*$/', $Line['text'], $matches)) {
            ...
            if (isset($matches[1])) {
                 ...
                 */
                $language = \Phabel\Target\Php80\Polyfill::substr($matches[1], 0, \Phabel\Target\Php80\Polyfill::strcspn($matches[1], " \t\n\f\r"));
    ...
    protected function blockFencedCodeContinue($Line, $Block)
    {
        ...
        if (preg_match('/^' . $Block['char'] . '{3,}[ ]*$/', $Line['text'])) {
            $Block['element']['text']['text'] = \Phabel\Target\Php80\Polyfill::substr($Block['element']['text']['text'], 1);
            $Block['complete'] = true;
            return $Block;
        }
        $Block['element']['text']['text'] .= "\n" . $Line['body'];
    ...
    protected function blockMarkup($Line)
    {
        ...
        if (preg_match('/^<(\\w[\\w-]*)(?:[ ]*' . $this->regexHtmlAttribute . ')*[ ]*(\\/)?>/', $Line['text'], $matches)) {
            ...
            $remainder = \Phabel\Target\Php80\Polyfill::substr($Line['text'], $length);
            if (trim($remainder) === '') {
    ...
    protected function blockTable($Line, array $Block = NULL)
    {
        ...
        if (strpos($Block['element']['text'], '|') !== false and chop($Line['text'], ' -:|') === '') {
            ...
            foreach ($dividerCells as $dividerCell) {
                ...
                if (\Phabel\Target\Php80\Polyfill::substr($dividerCell, -1) === ':') {
    ...
    public function line($text, $nonNestables = array())
    {
        ...
        while ($excerpt = strpbrk($text, $this->inlineMarkerList)) {
            ...
            foreach ($this->InlineTypes[$marker] as $inlineType) {
                ...
                $unmarkedText = \Phabel\Target\Php80\Polyfill::substr($text, 0, $Inline['position']);
                # compile the unmarked text
                $markup .= $this->unmarkedText($unmarkedText);
                # compile the inline
                $markup .= isset($Inline['markup']) ? $Inline['markup'] : $this->element($Inline['element']);
                # remove the examined text
                $text = \Phabel\Target\Php80\Polyfill::substr($text, $Inline['position'] + $Inline['extent']);
                continue 2;
            }
            # the marker does not belong to an inline
            $unmarkedText = \Phabel\Target\Php80\Polyfill::substr($text, 0, $markerPosition + 1);
            $markup .= $this->unmarkedText($unmarkedText);
            $text = \Phabel\Target\Php80\Polyfill::substr($text, $markerPosition + 1);
        }
    ...
    protected function inlineImage($Excerpt)
    {
        ...
        $Excerpt['text'] = \Phabel\Target\Php80\Polyfill::substr($Excerpt['text'], 1);
    ...
    protected function inlineLink($Excerpt)
    {
        ...
        if (preg_match('/\\[((?:[^][]++|(?R))*+)\\]/', $remainder, $matches)) {
            ...
            $remainder = \Phabel\Target\Php80\Polyfill::substr($remainder, $extent);
        } else {
            return;
        }
        if (preg_match('/^[(]\\s*+((?:[^ ()]++|[(][^ )]+[)])++)(?:[ ]+("[^"]*"|\'[^\']*\'))?\\s*[)]/', $remainder, $matches)) {
            ...
            if (isset($matches[2])) {
                $Element['attributes']['title'] = \Phabel\Target\Php80\Polyfill::substr($matches[2], 1, -1);
            }
            $extent += strlen($matches[0]);
        } else {
    ...
    protected function li($lines)
    {
        ...
        if (!in_array('', $lines) and \Phabel\Target\Php80\Polyfill::substr($trimmedMarkup, 0, 3) === '<p>') {
            $markup = $trimmedMarkup;
            $markup = \Phabel\Target\Php80\Polyfill::substr($markup, 3);
            $position = strpos($markup, "</p>");
            $markup = \Phabel\Target\Php80\Polyfill::substr_replace($markup, '', $position, 4);
        }
        return $markup;
    }
    #
    ...
    protected static function striAtStart($string, $needle)
    {
        ...
        if ($len > strlen($string)) {
            ...
            return strtolower(\Phabel\Target\Php80\Polyfill::substr($string, 0, $len)) === strtolower($needle);
        }
    }
    /**
     *
