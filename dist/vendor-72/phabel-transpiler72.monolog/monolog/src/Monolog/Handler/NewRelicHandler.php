...
class NewRelicHandler extends AbstractProcessingHandler
{
     ...
     * @param bool $bubble
     */
    public function __construct($level = 400, $bubble = true, ?string $appName = NULL, bool $explodeArrays = false, ?string $transactionName = NULL)
    {
        if (!\is_bool($bubble)) {
            if (!(\is_bool($bubble) || \is_numeric($bubble) || \is_string($bubble))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($bubble) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($bubble) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $bubble = (bool) $bubble;
            }
        }
