...
class TelegramBotHandler extends AbstractProcessingHandler
{
     ...
     */
    private const AVAILABLE_PARSE_MODES = array(0 => 'HTML', 1 => 'MarkdownV2', 2 => 'Markdown');
    /**
     ...
     */
    public function __construct($apiKey, $channel, $level = 100, bool $bubble = true, string $parseMode = NULL, bool $disableWebPagePreview = NULL, bool $disableNotification = NULL, bool $splitLongMessages = false, bool $delayBetweenMessages = false)
    {
        if (!\is_string($apiKey)) {
            if (!(\is_string($apiKey) || \is_object($apiKey) && \method_exists($apiKey, '__toString') || (\is_bool($apiKey) || \is_numeric($apiKey)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($apiKey) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($apiKey) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
