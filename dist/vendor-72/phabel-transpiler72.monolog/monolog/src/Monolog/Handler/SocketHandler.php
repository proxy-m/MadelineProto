...
class SocketHandler extends AbstractProcessingHandler
{
    ...
    * @param bool $bubble
    */
    public function __construct($connectionString, $level = 100, $bubble = true, bool $persistent = false, $timeout = 0.0, float $writingTimeout = 10.0, ?float $connectionTimeout = NULL, ?int $chunkSize = NULL)
    {
        if (!\is_string($connectionString)) {
            if (!(\is_string($connectionString) || \is_object($connectionString) && \method_exists($connectionString, '__toString') || (\is_bool($connectionString) || \is_numeric($connectionString)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #1 ($connectionString) must be of type string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($connectionString) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
