...
class AmqpHandler extends AbstractProcessingHandler
{
    ...
    */
     ...
     */
    public function __construct($exchange, $exchangeName = NULL, $level = 100, bool $bubble = true)
    {
        if (!(\is_string($exchangeName) || \is_null($exchangeName))) {
            if (!(\is_string($exchangeName) || \is_object($exchangeName) && \method_exists($exchangeName, '__toString') || (\is_bool($exchangeName) || \is_numeric($exchangeName)))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($exchangeName) must be of type ?string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($exchangeName) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
