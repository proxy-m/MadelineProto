...
class SamplingHandler extends AbstractHandler implements ProcessableHandlerInterface, FormattableHandlerInterface
{
    ...
    {
        if (!\is_int($factor)) {
            if (!(\is_bool($factor) || \is_numeric($factor))) {
                throw new \TypeError(__METHOD__ . '(): Argument #2 ($factor) must be of type int, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($factor) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $factor = (int) $factor;
            }
        }
