...
class StreamHandler extends AbstractProcessingHandler
{
    ...
    /** @const int 10MB */
    protected const DEFAULT_CHUNK_SIZE = 10485760;
    /** @var int */
     ...
     */
    public function __construct($stream, $level = 100, bool $bubble = true, ?int $filePermission = NULL, $useLocking = false)
    {
        if (!\is_bool($useLocking)) {
            if (!(\is_bool($useLocking) || \is_numeric($useLocking) || \is_string($useLocking))) {
                throw new \TypeError(__METHOD__ . '(): Argument #5 ($useLocking) must be of type bool, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($useLocking) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
            } else {
                $useLocking = (bool) $useLocking;
            }
        }
