...
class OverflowHandler extends AbstractHandler implements FormattableHandlerInterface
{
     ...
     */
    public function __construct($handler, $thresholdMap = array(), $level = 100, bool $bubble = true)
    {
        ...
        if (!\is_array($thresholdMap)) {
            throw new \TypeError(__METHOD__ . '(): Argument #2 ($thresholdMap) must be of type array, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($thresholdMap) . ' given, called in ' . \Phabel\Plugin\TypeHintReplacer::trace());
        }
