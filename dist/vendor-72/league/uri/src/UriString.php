...
final class UriString
{
     ...
     */
    private const URI_COMPONENTS = array('scheme' => NULL, 'user' => NULL, 'pass' => NULL, 'host' => NULL, 'port' => NULL, 'path' => '', 'query' => NULL, 'fragment' => NULL);
    /**
     * Simple URI which do not need any parsing.
     */
    private const URI_SCHORTCUTS = array('' => array(), '#' => array('fragment' => ''), '?' => array('query' => ''), '?#' => array('query' => '', 'fragment' => ''), '/' => array('path' => '/'), '//' => array('host' => ''));
    /**
    ...
    */
    ...
    private static function filterHost(string $host) : string
    {
        ...
        if (!self::isIpHost(\Phabel\Target\Php80\Polyfill::substr($host, 1, -1))) {
    ...
    private static function isIpHost(string $ip_host) : bool
    {
        ...
        $pos = strpos($ip_host, '%');
        if (false === $pos || 1 === preg_match(self::REGEXP_INVALID_HOST_CHARS, rawurldecode(\Phabel\Target\Php80\Polyfill::substr($ip_host, $pos)))) {
            return false;
        }
        $ip_host = \Phabel\Target\Php80\Polyfill::substr($ip_host, 0, $pos);
