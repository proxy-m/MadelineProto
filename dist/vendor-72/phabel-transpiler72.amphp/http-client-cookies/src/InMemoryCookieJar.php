...
final class InMemoryCookieJar implements CookieJar
{
    ...
    private function matchesDomain(string $requestDomain, string $cookieDomain) : bool
    {
        ...
        if (\Phabel\Target\Php80\Polyfill::substr($requestDomain, 0, -\strlen($cookieDomain)) . $cookieDomain === $requestDomain) {
            return true;
        }
        return false;
    }
    ...
    private function matchesPath(string $requestPath, string $cookiePath) : bool
    {
        ...
        if (\Phabel\Target\Php80\Polyfill::substr($cookiePath, -1) === '/' || $requestPath[\strlen($cookiePath)] === '/') {
            return true;
        }
        return false;
    }
