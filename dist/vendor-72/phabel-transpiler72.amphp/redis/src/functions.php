...
function connect(Config $config, ?Socket\Connector $connector = null) : Promise
{
    return call(static function () use($config, $connector) {
        try {
            ...
            $resp = new RespSocket((yield ($connector ?? Socket\connector())->connect($config->getConnectUri(), $connectContext)));
        } catch (Socket\SocketException $e) {
            throw new SocketException('Failed to connect to redis instance (' . $config->getConnectUri() . ')', 0, $e);
        }
        $promises = [];
