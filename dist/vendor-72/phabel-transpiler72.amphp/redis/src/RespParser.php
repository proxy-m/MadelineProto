        ...
        do {
            ...
            switch ($type) {
                ...
                case self::TYPE_ERROR:
                    $payload = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 1, $pos - 1);
                    $remove = $pos + 2;
                    break;
                case self::TYPE_BULK_STRING:
                    $length = (int) \Phabel\Target\Php80\Polyfill::substr($this->buffer, 1, $pos);
                    if ($length === -1) {
                        $payload = null;
                        $remove = $pos + 2;
                    } else {
                        ...
                        $payload = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $pos + 2, $length);
                        $remove = $pos + $length + 4;
                    }
                    break;
                default:
                    throw new ParserException(\sprintf('unknown resp data type: %s', $type));
            }
            $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $remove);
            switch ($type) {
                case self::TYPE_INTEGER:
                case self::TYPE_ARRAY:
                    $payload = (int) $payload;
