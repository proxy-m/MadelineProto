...
final class HPackNative
{
    ...
    public static function huffmanDecode(string $input)
    {
        ...
        return \Phabel\Target\Php80\Polyfill::substr($out, 0, $off);
    }
    /**
     *
     */
    ...
    public static function huffmanEncode(string $input) : string
    {
        ...
        return $i ? \Phabel\Target\Php80\Polyfill::substr($out, 0, $byte) : '';
    }
            ...
            while ($off < $inputLength) {
                ...
                if ($index & 0x80) {
                    ...
                    if ($index & ($dynamic ? 0x3f : 0xf)) {
                        ...
                        if ($huffman) {
                            $header = [self::huffmanDecode(\Phabel\Target\Php80\Polyfill::substr($input, $off, $length))];
                            if ($header[0] === null) {
                                return null;
                            }
                        } else {
                            $header = [\Phabel\Target\Php80\Polyfill::substr($input, $off, $length)];
                        }
                        $off += $length;
                    }
                    if ($off >= $inputLength) {
                    ...
                    if ($huffman) {
                        $header[1] = self::huffmanDecode(\Phabel\Target\Php80\Polyfill::substr($input, $off, $length));
                        if ($header[1] === null) {
                            return null;
                        }
                    } else {
                        $header[1] = \Phabel\Target\Php80\Polyfill::substr($input, $off, $length);
                    }
                    $off += $length;
                    if ($dynamic) {
                        \Phabel\Target\Php73\Polyfill::array_unshift($this->headers, $header);
