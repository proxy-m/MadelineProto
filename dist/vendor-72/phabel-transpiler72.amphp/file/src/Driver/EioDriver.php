...
final class EioDriver implements Driver
{
     ...
     *
     */
    public function createDirectory(string $path, int $mode = 511) : Promise
    {
        $deferred = new Deferred();
        $this->poll->listen($deferred->promise());
     ...
     *
     */
    public function createDirectoryRecursively(string $path, int $mode = 511) : Promise
    {
        $deferred = new Deferred();
        $this->poll->listen($deferred->promise());
        $priority = \EIO_PRI_DEFAULT;
        ...
        $tmpPath = "";
        $callback = function () use(&$callback, &$arrayPath, &$tmpPath, $mode, $priority, $deferred) : void {
    ...
    private function onOpenHandle(array $openArr, $result, $req) : void
    {
        ...
        if ($result === -1) {
            ...
            \Phabel\Target\Php73\Polyfill::array_unshift($openArr, $result);
    ...
    private function isDir(string $path) : Promise
    {
        ...
        $this->getStatus($path)->onResolve(static function ($error, $result) use($deferred) : void {
            if ($result) {
                $deferred->resolve(($result["mode"] & 040000) === 040000);
            } else {
