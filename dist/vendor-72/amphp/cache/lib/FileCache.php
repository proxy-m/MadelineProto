        ...
        $gcWatcher = static function () use($directory, $mutex, $ampFileVersion2) : \Generator {
            try {
                ...
                foreach ($files as $file) {
                    if (\strlen($file) !== 70 || \Phabel\Target\Php80\Polyfill::substr($file, -\strlen('.cache')) !== '.cache') {
                        continue;
                    }
                    /** @var Lock $lock */
    ...
    public function get(string $key) : Promise
    {
        return call(function () use($key) {
            ...
            try {
                ...
                $ttl = \unpack('Nttl', \Phabel\Target\Php80\Polyfill::substr($cacheContent, 0, 4))['ttl'];
                if ($ttl < \time()) {
                    /** @psalm-suppress UndefinedFunction */
                    (yield $this->ampFileVersion2 ? File\deleteFile($this->directory . '/' . $filename) : File\unlink($this->directory . '/' . $filename));
                    return null;
                }
                $value = \Phabel\Target\Php80\Polyfill::substr($cacheContent, 4);
