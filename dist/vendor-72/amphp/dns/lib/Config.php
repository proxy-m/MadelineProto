...
final class Config
{
     ...
     *
     */
    public function __construct(array $nameservers, array $knownHosts = array(), int $timeout = 3000, int $attempts = 2)
    {
    ...
    private function validateNameserver($nameserver)
    {
        ...
        if ($nameserver[0] === "[") {
            // IPv6
            $addr = \strstr(\Phabel\Target\Php80\Polyfill::substr($nameserver, 1), "]", true);
            $port = \Phabel\Target\Php80\Polyfill::substr($nameserver, \strrpos($nameserver, "]") + 1);
            if ($port !== "" && !\preg_match("(^:(\\d+)\$)", $port, $match)) {
                throw new ConfigException("Invalid nameserver: {$nameserver}");
            }
            $port = $port === "" ? 53 : \Phabel\Target\Php80\Polyfill::substr($port, 1);
        } else {
            // IPv4
