...
final class Http1Parser
{
    ...
    public function parse(string $data = NULL) : ?Response
    {
        ...
        $startLine = \Phabel\Target\Php80\Polyfill::substr($startLineAndHeaders, 0, $startLineEndPos);
        $rawHeaders = \Phabel\Target\Php80\Polyfill::substr($startLineAndHeaders, $startLineEndPos + 2);
        ...
        $bodyData = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $this->remainingBodyBytes);
        $this->addToBody($bodyData);
        $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $this->remainingBodyBytes);
        ...
        return null;
        trailers_start:
        $firstTwoBytes = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, 2);
        if ($firstTwoBytes === "" || $firstTwoBytes === "\r") {
            return null;
        }
        if ($firstTwoBytes === "\r\n") {
            $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 2);
            goto complete;
        }
    ...
    private function shiftHeadersFromBuffer() : ?string
    {
        ...
        if ($headersSize = \strpos($this->buffer, "\r\n\r\n")) {
            $headers = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $headersSize + 2);
            $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $headersSize + 4);
        } else {
    ...
    private function parseChunkedBody() : bool
    {
        ...
        $line = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $lineEndPos);
        ...
        $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $lineEndPos + 2);
        ...
        if ($bufferLength >= $this->chunkLengthRemaining + 2) {
            $chunk = \Phabel\Target\Php80\Polyfill::substr($this->buffer, 0, $this->chunkLengthRemaining);
            $this->buffer = \Phabel\Target\Php80\Polyfill::substr($this->buffer, $this->chunkLengthRemaining + 2);
