...
abstract class DriverTest extends FilesystemTest
{
    ...
    {
        return ['nonExistingPath' => [static function () {
            return Fixture::path() . '/' . \uniqid('amphp-test-', true);
        }], 'notLink' => [static function () {
            return Fixture::path();
        }]];
    }
    /**
    ...
    public function testChangePermissions() : \Generator
    {
        ...
        $path = "{$fixtureDir}/file";
        $stat = (yield $this->driver->getStatus($path));
        $this->assertNotSame('0777', \Phabel\Target\Php80\Polyfill::substr(\decoct($stat['mode']), -4));
        $this->assertNull((yield $this->driver->changePermissions($path, 0777)));
        $stat = (yield $this->driver->getStatus($path));
        $this->assertSame('0777', \Phabel\Target\Php80\Polyfill::substr(\decoct($stat['mode']), -4));
    }
    /**
     *
     */
     ...
     *
     */
    protected abstract function createDriver() : File\Driver;
    /**
     *
     */
    private function assertSameStatus(array $expected, array $actual) : void
    {
        $filter = function (array $stat) {
            $filtered = \Phabel\Target\Php74\Polyfill::array_filter($stat, function (string $key) : bool {
    ...
    {
        return \Phabel\Target\Php80\Polyfill::substr(\decoct($stat["mode"]), 1);
    }
}