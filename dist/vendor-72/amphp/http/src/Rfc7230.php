...
final class Rfc7230
{
    ...
    public const HEADER_NAME_REGEX = '(^([^()<>@,;:\\"/[\\]?={}- ]++)$)';
    public const HEADER_VALUE_REGEX = '(^[ 	]*+((?:[ 	]*+[!-~€-ÿ]++)*+)[ 	]*+$)';
    public const HEADER_REGEX = '(^([^()<>@,;:\\"/[\\]?={}- ]++):[ 	]*+((?:[ 	]*+[!-~€-ÿ]++)*+)[ 	]*+
)m';
...
[ 	]++)';
    ...
    */
    public static function formatRawHeaders(array $headers) : string
    {
        ...
        if ($lines !== $count || $lines !== \Phabel\Target\Php80\Polyfill::substr_count($buffer, "\n")) {
