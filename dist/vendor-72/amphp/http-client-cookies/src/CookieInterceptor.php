    ...
    private function assignApplicableRequestCookies(Request $request) : \Generator
    {
        ...
        if ($cookiePairs) {
            if ($request->hasHeader('cookie')) {
                \Phabel\Target\Php73\Polyfill::array_unshift($cookiePairs, $request->getHeader('cookie'));
            }
        ...
        try {
            ...
            if (!$cookie->getDomain()) {
                ...
                if ($cookieDomain !== $requestDomain) {
                    ...
                    if (\Phabel\Target\Php80\Polyfill::substr($requestDomain, 0, -$cookieDomainLength - 1) . '.' . $cookieDomain !== $requestDomain) {
                        return null;
                    }
                }
