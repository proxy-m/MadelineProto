...
{
    private const LOCK = 'local lock = KEYS[1]
local queue = KEYS[2]
...
if redis.call("exists", lock) == 0 then
    ...
    private const UNLOCK = 'local lock = KEYS[1]
local token = ARGV[1]
...
if redis.call("get", lock) == token then
    ...
    private const RENEW = 'for i=1,#KEYS do
     ...
     */
    public function __construct(QueryExecutorFactory $queryExecutorFactory, ?MutexOptions $options = NULL, ?PsrLogger $logger = NULL)
    {
    ...
    public function acquire(string $key) : Promise
    {
        return call(function () use($key) {
            ...
            do {
                ...
                $result = (yield $this->sharedConnection->eval(self::LOCK, ["{$prefix}lock:{$key}", "{$prefix}lock-queue:{$key}"], [$token, $this->options->getLockExpiration(), $this->options->getLockExpiration() + $this->options->getLockTimeout()]));
                if ($result < 1) {
    ...
    private function unlock(string $key, string $token) : Promise
    {
        return call(function () use($key, $token) {
            ...
            for ($attempt = 0; $attempt < 2; $attempt++) {
                try {
                    $result = (yield $this->sharedConnection->eval(self::UNLOCK, ["{$prefix}lock:{$key}"], [$token]));
                    if ($result === 2) {
                        $this->logger->warning('Lock was already expired when unlocked', ['key' => $key]);
                    }
                    break;
                } catch (RedisException $e) {
                    $this->logger->error('Unlock operation failed on attempt ' . ($attempt + 1), ['exception' => $e]);
                }
            }
        });
    }
    ...
    private function createRenewWatcher() : void
    {
        $this->watcher = Loop::repeat($this->options->getLockRenewInterval(), function () {
            ...
            try {
                ...
                $this->logger->error('Renew operation failed, locks might expire', ['exception' => $e]);
            }
        });
    }
}