...
class Polyfill extends Plugin
{
    ...
    public static function mb_convert_encoding($string, string $to_encoding, $from_encoding = NULL)
    {
        ...
        if (\is_array($string)) {
            ...
            $phabelReturn = $string;
            if (!(\is_array($phabelReturn) || \is_bool($phabelReturn) || \is_string($phabelReturn))) {
                if (!(\is_string($phabelReturn) || \is_object($phabelReturn) && \method_exists($phabelReturn, '__toString') || (\is_bool($phabelReturn) || \is_numeric($phabelReturn)))) {
                    if (!(\is_bool($phabelReturn) || \is_numeric($phabelReturn) || \is_string($phabelReturn))) {
                        throw new \TypeError(__METHOD__ . '(): Return value must be of type array|bool|string, ' . \Phabel\Plugin\TypeHintReplacer::getDebugType($phabelReturn) . ' returned in ' . \Phabel\Plugin\TypeHintReplacer::trace());
                    }
