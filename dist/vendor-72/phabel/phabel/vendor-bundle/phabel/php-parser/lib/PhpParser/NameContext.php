...
class NameContext
{
     ...
     */
    public function addAlias(Name $name, string $aliasName, int $type, array $errorAttrs = array())
    {
    ...
    public function getPossibleNames(string $name, int $type) : array
    {
        ...
        foreach ($this->origAliases[Stmt\Use_::TYPE_NORMAL] as $alias => $orig) {
            ...
            if (0 === \strpos($lcName, $lcOrig . '\\')) {
                $possibleNames[] = new Name($alias . \Phabel\Target\Php80\Polyfill::substr($name, \strlen($lcOrig)));
            }
        }
    ...
    private function getNamespaceRelativeName(string $name, string $lcName, int $type)
    {
        ...
        if (0 === \strpos($lcName, $namespacePrefix)) {
            return new Name(\Phabel\Target\Php80\Polyfill::substr($name, \strlen($namespacePrefix)));
        }
        return null;
    }
    /**
    ...
    private function normalizeConstName(string $name)
    {
        ...
        $ns = \Phabel\Target\Php80\Polyfill::substr($name, 0, $nsSep);
        $shortName = \Phabel\Target\Php80\Polyfill::substr($name, $nsSep + 1);
