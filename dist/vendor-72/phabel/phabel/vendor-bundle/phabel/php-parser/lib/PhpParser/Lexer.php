...
use PhabelVendor\PhpParser\Parser\Tokens;
class Lexer
{
    protected $code;
    protected $tokens;
    ...
    */
    ...
    {
        return ($token[0] === \T_COMMENT || $token[0] === \T_DOC_COMMENT) && \Phabel\Target\Php80\Polyfill::substr($token[1], 0, 2) === '/*' && \Phabel\Target\Php80\Polyfill::substr($token[1], -2) !== '*/';
    }
    /**
     *
     */
    protected function postprocessTokens(ErrorHandler $errorHandler)
    {
        ...
        for ($i = 0; $i < $numTokens; $i++) {
            ...
            if ($token[0] === \T_COMMENT && \Phabel\Target\Php80\Polyfill::substr($token[1], 0, 2) !== '/*' && \preg_match('/(\\r\\n|\\n|\\r)$/D', $token[1], $matches)) {
                $trailingNewline = $matches[0];
                $token[1] = \Phabel\Target\Php80\Polyfill::substr($token[1], 0, -\strlen($trailingNewline));
                $this->tokens[$i] = $token;
                if (isset($this->tokens[$i + 1]) && $this->tokens[$i + 1][0] === \T_WHITESPACE) {
                    ...
                    \Phabel\Target\Php74\Polyfill::array_splice($this->tokens, $i + 1, 0, [[\T_WHITESPACE, $trailingNewline, $line]]);
                    $numTokens++;
                }
            }
            ...
            if (\is_array($token) && ($token[0] === \T_NS_SEPARATOR || isset($this->identifierTokens[$token[0]]))) {
                ...
                if ($lastWasSeparator) {
                    ...
                    $j--;
                    $text = \Phabel\Target\Php80\Polyfill::substr($text, 0, -1);
                }
                if ($j > $i + 1) {
                    ...
                    \Phabel\Target\Php74\Polyfill::array_splice($this->tokens, $i, $j - $i, [$token]);
                    $numTokens -= $j - $i - 1;
                }
            }
            if ($token === '&') {
            ...
            if (\Phabel\Target\Php80\Polyfill::substr($this->code, $filePos, $tokenLen) !== $tokenValue) {
                // Something is missing, must be an invalid character
                $nextFilePos = \strpos($this->code, $tokenValue, $filePos);
                $badCharTokens = $this->handleInvalidCharacterRange($filePos, $nextFilePos, $line, $errorHandler);
                $filePos = (int) $nextFilePos;
                \Phabel\Target\Php74\Polyfill::array_splice($this->tokens, $i, 0, $badCharTokens);
                $numTokens += \count($badCharTokens);
                $i += \count($badCharTokens);
            }
            $filePos += $tokenLen;
            $line += \Phabel\Target\Php80\Polyfill::substr_count($tokenValue, "\n");
        }
        if ($filePos !== \strlen($this->code)) {
            if (\Phabel\Target\Php80\Polyfill::substr($this->code, $filePos, 2) === '/*') {
                // Unlike PHP, HHVM will drop unterminated comments entirely
                $comment = \Phabel\Target\Php80\Polyfill::substr($this->code, $filePos);
                $errorHandler->handleError(new Error('Unterminated comment', ['startLine' => $line, 'endLine' => $line + \Phabel\Target\Php80\Polyfill::substr_count($comment, "\n"), 'startFilePos' => $filePos, 'endFilePos' => $filePos + \strlen($comment)]));
        ...
        if (\count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new Error('Unterminated comment', ['startLine' => $line - \Phabel\Target\Php80\Polyfill::substr_count($lastToken[1], "\n"), 'endLine' => $line, 'startFilePos' => $filePos - \strlen($lastToken[1]), 'endFilePos' => $filePos]));
            }
        }
    }
    /**
    ...
    public function getNextToken(&$value = NULL, &$startAttributes = NULL, &$endAttributes = NULL) : int
    {
        ...
        while (1) {
            ...
            if (\is_string($token)) {
                ...
                $this->line += \Phabel\Target\Php80\Polyfill::substr_count($value, "\n");
                $this->filePos += \strlen($value);
            } else {
                $origLine = $this->line;
                $origFilePos = $this->filePos;
                $this->line += \Phabel\Target\Php80\Polyfill::substr_count($token[1], "\n");
    ...
    */
    ...
    public function handleHaltCompiler() : string
    {
        ...
        $textAfter = \Phabel\Target\Php80\Polyfill::substr($this->code, $this->filePos);
        ...
        // return with (); removed
        return \Phabel\Target\Php80\Polyfill::substr($textAfter, \strlen($matches[0]));
    }
    /**
     *
     */
