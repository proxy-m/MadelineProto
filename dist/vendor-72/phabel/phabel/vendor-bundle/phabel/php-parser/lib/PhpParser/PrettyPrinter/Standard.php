...
namespace PhabelVendor\PhpParser\PrettyPrinter;
...
class Standard extends PrettyPrinterAbstract
{
    ...
    {
        return $this->pAttrGroups($node->attrGroups, \true) . ($node->static ? 'static ' : '') . 'fn' . ($node->byRef ? '&' : '') . '(' . $this->pCommaSeparated($node->params) . ')' . (null !== $node->returnType ? ': ' . $this->p($node->returnType) : '') . ' => ' . $this->pDereferenceLhs($node->expr);
    }
    /**
     *
     */
