...
use PhabelVendor\Amp\Process\Internal\ProcessRunner;
...
{
    const FD_SPEC = array(0 => array(0 => 'pipe', 1 => 'r'), 1 => array(0 => 'pipe', 1 => 'w'), 2 => array(0 => 'pipe', 1 => 'w'), 3 => array(0 => 'pipe', 1 => 'w'));
    /** @var string|null */
     ...
     * @inheritdoc
     */
    public function start(string $command, string $cwd = NULL, array $env = array(), array $options = array()) : ProcessHandle
    {
    ...
    private function generateFds() : array
    {
        ...
        $fdList = \Phabel\Target\Php74\Polyfill::array_filter($fdList, function (string $path) : bool {
            return $path !== "." && $path !== "..";
        });
        $fds = [];
