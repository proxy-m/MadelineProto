...
use PhabelVendor\Amp\Parallel\Sync\ChannelException;
use PhabelVendor\Amp\Parallel\Sync\ChannelledSocket;
use PhabelVendor\Amp\Parallel\Sync\ExitResult;
use PhabelVendor\Amp\Parallel\Sync\SynchronizationError;
use PhabelVendor\Amp\Process\Process as BaseProcess;
...
use PhabelVendor\Amp\TimeoutException;
...
final class Process implements Context
{
    ...
    */
    public static function run($script, string $cwd = NULL, array $env = array(), string $binary = NULL) : Promise
    {
    ...
    */
    public function __construct($script, string $cwd = NULL, array $env = array(), string $binary = NULL)
    {
        ...
        if (\strpos(self::SCRIPT_PATH, "phar://") === 0) {
            if (self::$pharScriptPath) {
                ...
                if (\Phabel\Target\Php80\Polyfill::substr(\Phar::running(\false), -5) !== ".phar") {
                    ...
                    $path = "phar://" . self::$pharCopy . "/" . \Phabel\Target\Php80\Polyfill::substr($path, \strlen(\Phar::running(\true)));
                }
            ...
            if (isset(self::$pharCopy) && \is_array($script) && isset($script[0])) {
                $script[0] = "phar://" . self::$pharCopy . \Phabel\Target\Php80\Polyfill::substr($script[0], \strlen(\Phar::running(\true)));
            }
        } else {
    ...
    private static function locateBinary() : string
    {
        ...
        $paths = \Phabel\Target\Php74\Polyfill::array_filter(\explode(\PATH_SEPARATOR, \getenv("PATH")));
        $paths[] = \PHP_BINDIR;
