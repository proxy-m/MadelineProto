...
use PhabelVendor\Symfony\Component\Console\Formatter\WrappableOutputFormatterInterface;
...
class Table
{
    ...
    */
     ...
     * +-----+-----------+-------+
     */
    private function renderRowSeparator(int $type = 2, string $title = NULL, string $titleFormat = NULL)
    {
        ...
        if (null !== $title) {
            ...
            if (\false === \mb_detect_encoding($markup, null, \true)) {
                $markup = \Phabel\Target\Php80\Polyfill::substr_replace($markup, $formattedTitle, $titleStart, $titleLength);
            } else {
     ...
     */
    private function renderColumnSeparator(int $type = 0) : string
    {
        $borders = $this->style->getBorderChars();
    ...
    private function fillNextRows(array $rows, int $line) : array
    {
        ...
        foreach ($rows[$line] as $column => $cell) {
            ...
            if ($cell instanceof TableCell && $cell->getRowspan() > 1) {
                ...
                if (\strstr($cell, "\n")) {
                    $lines = \explode("\n", \str_replace("\n", "<fg=default;bg=default>\n</>", $cell));
                    $nbLines = \count($lines) > $nbLines ? \Phabel\Target\Php80\Polyfill::substr_count($cell, "\n") : $nbLines;
        ...
        foreach ($unmergedRows as $unmergedRowKey => $unmergedRow) {
            ...
            if (isset($rows[$unmergedRowKey]) && \is_array($rows[$unmergedRowKey]) && $this->getNumberOfColumns($rows[$unmergedRowKey]) + $this->getNumberOfColumns($unmergedRows[$unmergedRowKey]) <= $this->numberOfColumns) {
                foreach ($unmergedRow as $cellKey => $cell) {
                    ...
                    \Phabel\Target\Php74\Polyfill::array_splice($rows[$unmergedRowKey], $cellKey, 0, [$cell]);
                }
            } else {
                ...
                \Phabel\Target\Php74\Polyfill::array_splice($rows, $unmergedRowKey, 0, [$row]);
            }
        }
        return $rows;
    }
