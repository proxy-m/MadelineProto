...
use PhabelVendor\Symfony\Component\Console\Cursor;
use PhabelVendor\Symfony\Component\Console\Exception\MissingInputException;
...
use PhabelVendor\Symfony\Component\Console\Input\StreamableInputInterface;
use PhabelVendor\Symfony\Component\Console\Output\ConsoleOutputInterface;
use PhabelVendor\Symfony\Component\Console\Output\ConsoleSectionOutput;
use PhabelVendor\Symfony\Component\Console\Output\OutputInterface;
use PhabelVendor\Symfony\Component\Console\Question\ChoiceQuestion;
use PhabelVendor\Symfony\Component\Console\Question\Question;
use PhabelVendor\Symfony\Component\Console\Terminal;
use function PhabelVendor\Symfony\Component\String\s;
/**
...
class QuestionHelper extends Helper
{
    ...
    private function autocomplete(OutputInterface $output, Question $question, $inputStream, callable $autocomplete) : string
    {
        ...
        while (!\feof($inputStream)) {
            ...
            if (\false === $c || '' === $ret && '' === $c && null === $question->getDefault()) {
                ...
                if ("\t" === $c || "\n" === $c) {
                    if ($numMatches > 0 && -1 !== $ofs) {
                        ...
                        $remainingCharacters = \Phabel\Target\Php80\Polyfill::substr($ret, \strlen(\trim($this->mostRecentlyEnteredValue($fullChoice))));
                        $output->write($remainingCharacters);
                        $fullChoice .= $remainingCharacters;
                        $i = \false === ($encoding = \mb_detect_encoding($fullChoice, null, \true)) ? \strlen($fullChoice) : \mb_strlen($fullChoice, $encoding);
                        $matches = \Phabel\Target\Php74\Polyfill::array_filter($autocomplete($ret), function ($match) use($ret) {
            ...
            if ($numMatches > 0 && -1 !== $ofs) {
                ...
                $output->write('<hl>' . OutputFormatter::escapeTrailingBackslash(\Phabel\Target\Php80\Polyfill::substr($matches[$ofs], $charactersEntered)) . '</hl>');
    ...
    private function getHiddenResponse(OutputInterface $output, $inputStream, bool $trimmable = true) : string
    {
        if ('\\' === \DIRECTORY_SEPARATOR) {
            ...
            if ('phar:' === \Phabel\Target\Php80\Polyfill::substr(__FILE__, 0, 5)) {
