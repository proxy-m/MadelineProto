 ...
 */
namespace PhabelVendor\Symfony\Component\Console\Command;
...
use PhabelVendor\Symfony\Component\Console\Completion\Output\BashCompletionOutput;
use PhabelVendor\Symfony\Component\Console\Completion\Output\CompletionOutputInterface;
...
final class CompleteCommand extends Command
{
     ...
     */
    public function __construct(array $completionOutputs = array())
    {
    ...
    protected function execute(InputInterface $input, OutputInterface $output) : int
    {
        try {
            ...
            $this->log(['', '<comment>' . \Phabel\Target\Php80\Polyfill::date('Y-m-d H:i:s') . '</>', '<info>Input:</> <comment>("|" indicates the cursor position)</>', '  ' . (string) $completionInput, '<info>Command:</>', '  ' . (string) \implode(' ', $_SERVER['argv']), '<info>Messages:</>']);
            ...
            if (null === $command) {
                ...
                $suggestions->suggestValues(\Phabel\Target\Php74\Polyfill::array_filter(\array_merge([$command->getName()], $command->getAliases())));
            } else {
