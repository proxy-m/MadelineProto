...
foreach (Php::VERSIONS as $version) {
    $types = ['callable', 'iterable', 'object', 'self', 'static', 'int', 'float', 'array', 'string', 'bool', \Generator::class, \str_replace("Target", "Target{$version}", \PhabelTest\Target\TypeHintReplacerTest::class)];
    ...
    $packages += (new Traverser(EventHandler::create()))->setPlugins([PhabelTestGenerator::class => ['target' => $version], TypeHintReplacer::class => ['union' => true, 'nullable' => true, 'return' => true, 'types' => $types]])->setInput('testsGenerated/Target')->setOutput("testsGenerated/Target{$version}")->setCoverage("expr{$version}")->runAsync();
    $packagesSecondary += (new Traverser(EventHandler::create()))->setPlugins([PhabelTestGenerator::class => ['target' => 1000 + $version]])->setInput("testsGenerated/Target{$version}")->setOutput("testsGenerated/Target10{$version}")->setCoverage("expr10{$version}")->runAsync();
}
