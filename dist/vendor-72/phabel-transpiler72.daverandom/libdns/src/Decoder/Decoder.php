    ...
    private function decodeDomainName(DecodingContext $decodingContext, DomainName $domainName) : int
    {
        ...
        while (++$totalLength && ($length = \ord($this->readDataFromPacket($packet, 1)))) {
            ...
            if ($labelType === 0b0) {
                ...
                \Phabel\Target\Php73\Polyfill::array_unshift($labels, [$index, $label]);
                $totalLength += $length;
            } else {
                if ($labelType === 0b11000000) {
                    ...
                    \Phabel\Target\Php73\Polyfill::array_unshift($labels, $ref);
                    $totalLength++;
                    break;
                } else {
        ...
        foreach ($labels as $label) {
            if (\is_int($label[0])) {
                \Phabel\Target\Php73\Polyfill::array_unshift($result, $label[1]);
