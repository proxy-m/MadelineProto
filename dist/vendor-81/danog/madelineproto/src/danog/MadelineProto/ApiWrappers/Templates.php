            ...
            if (!$this->myTelegramOrgWrapper->loggedIn()) {
                ...
                foreach (['app_title', 'app_shortname', 'app_url', 'app_platform', 'app_desc'] as $k => $field) {
                    ...
                    if ($field == 'app_platform') {
                        $form .= "{$desc}<br>";
                        foreach (['android' => 'Android', 'ios' => 'iOS', 'wp' => 'Windows Phone', 'bb' => 'BlackBerry', 'desktop' => 'Desktop', 'web' => 'Web', 'ubp' => 'Ubuntu phone', 'other' => \htmlentities(Lang::$current_lang['apiAppInstructionsAutoTypeOther'])] as $key => $desc) {
